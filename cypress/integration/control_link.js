context("Control Link", () => {
	...
	it("should unset invalid value", () => {
		...
		cy.intercept("/api/method/nts.client.validate_link_and_fetch*").as("validate_link");
		cy.get(".nts-control[data-fieldname=link] input").focus().as("input");
		// Wait for dropdown to appear (request might be cached)
		cy.get("@input").parent().findByRole("listbox").should("be.visible");
		cy.wait(200);
	...
	it("should update dependant fields (via fetch_from)", () => {
		cy.get("@todos").then((todos) => {
			...
			cy.call("nts.client.get_value", {
				...
				cy.get(
					".nts-control[data-fieldname=assigned_by_full_name] .control-value"
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("nts.tests.ui_test_helpers.insert_translations").then(() => {
			...
			cy.wait(500);
			cy.get(".nts-control[data-fieldname=link] ul").should("be.visible");
			cy.get(".nts-control[data-fieldname=link] input").type("{enter}");
