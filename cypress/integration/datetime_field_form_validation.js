context("Datetime Field Validation", () => {
	before(() => {
		...
		cy.window().should("have.property", "nts");
		return cy
			.window()
			.its("nts")
			.then((nts) => {
				return nts.xcall("nts.tests.ui_test_helpers.create_datetime_test_doctype");
			});
	});
	...
	it("datetime field form validation", () => {
		...
		cy.window()
			.its("nts")
			.then((nts) => {
				return nts.xcall("nts.tests.ui_test_helpers.create_datetime_test_record");
			})
			.then((doc) => {
