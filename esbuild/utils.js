...
function get_cloned_apps() {
	/**
	 * Returns nts apps in the bench/apps folder
	 */
	const apps = [];
	for (const app of fs.readdirSync(apps_path)) {
		const app_path = path.resolve(apps_path, app);
		if (is_nts_app(app, app_path)) apps.push(app);
	}

	return apps;
}

function is_nts_app(app_name, app_path) {
	/**
	 * Same as the is_nts_app check in nts/bench
	 */
