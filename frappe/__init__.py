...
from nts.utils.caching import deprecated_local_cache as local_cache
from nts.utils.caching import request_cache, site_cache
from nts.utils.data import as_unicode, bold, cint, cstr, safe_decode, safe_encode, sbool
from nts.utils.local import Local, LocalProxy, release_local
from nts.utils.translations import _, _lt, set_user_lang
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from nts.database.mariadb.database import MariaDBDatabase as PyMariaDBDatabase
	from nts.database.mariadb.mysqlclient import MariaDBDatabase
	from nts.database.postgres.database import PostgresDatabase
	from nts.database.sqlite.database import SQLiteDatabase
	from nts.model.document import Document
	from nts.query_builder.builder import MariaDB, Postgres, SQLite
	from nts.utils.redis_wrapper import ClientCache, RedisWrapper
...
# this global may be subsequently changed by nts.tests.utils.toggle_test_mode()
in_test = False
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(nts.request))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `nts.init`.
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling nts.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke nts.init(site) prior to calling nts.connect(), if initializing the site is necessary.",
		)
		init(site)
	...
	if db_name:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling nts.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke nts.init(site) with the right config prior to calling nts.connect(), if necessary.",
		)
	...
	if nts.conf.db_type in ("mariadb", "postgres"):
...
def setup_redis_cache_connection():
	...
	from nts.utils.redis_wrapper import ClientCache, setup_cache
...
def print_sql(enable: bool = True) -> None:
	if not local.conf.allow_tests:
		nts.throw("`nts.print_sql` only works with `allow_tests` site config enabled.")
...
def has_permission(
	...
	*,
	...
	Raise `nts.PermissionError` if user isn't permitted and `throw` is truthy
...
def get_list(doctype, *args, **kwargs):
	...
	"""
	import nts.model.qb_query

	return nts.model.qb_query.DatabaseQuery(doctype).execute(*args, **kwargs)
...
from nts.deprecation_dumpster import nts_get_test_records as get_test_records
...
def is_setup_complete():
	...
	if not nts.db.table_exists("Installed Application"):
		return setup_complete

	if all(
		nts.get_all(
			"Installed Application",
			{"app_name": ("in", ["nts", "erpnext"])},
...
def override_whitelisted_method(original_method: str) -> str:
	...
	overrides = nts.get_hooks("override_whitelisted_methods", {}).get(original_method, [])
	return overrides[-1] if overrides else original_method


# Backward compatibility
from nts.utils.messages import *  # noqa: I001

import nts._optimizations
from nts.cache_manager import clear_cache, reset_metadata_version
from nts.config import get_common_site_config, get_conf, get_site_config
from nts.core.doctype.system_settings.system_settings import get_system_settings
from nts.model.document import (
	get_doc,
	get_lazy_doc,
	copy_doc,
	new_doc,
...
from nts.model.meta import get_meta
from nts.realtime import publish_progress, publish_realtime
from nts.utils import get_traceback, mock, parse_json, safe_eval, create_folder
from nts.utils.background_jobs import enqueue, enqueue_doc
from nts.utils.error import log_error
from nts.utils.formatters import format_value
from nts.utils.print_utils import get_print, attach_print
from nts.email import sendmail

# for backwards compatibility
format = format_value
delete_doc_if_exists = delete_doc

nts._optimizations.optimize_all()
nts._optimizations.register_fault_handler()
