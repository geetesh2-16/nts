...
from nts import _
from nts.auth import SAFE_HTTP_METHODS, UNSAFE_HTTP_METHODS, HTTPRequest, check_request_ip, validate_auth
from nts.integrations.oauth2 import get_resource_url, handle_wellknown, is_oauth_metadata_enabled
...
from nts.utils.error import log_error, log_error_snapshot
...
import nts.database.mariadb.mysqlclient  # Load database related utils
...
# this is necessary because nts desk sends most requests as form data
...
def application(request: Request):
	...
	try:
		...
		if db := getattr(nts.local, "db", None):
...
def init_request(request):
	...
	nts.init(site, sites_path=_sites_path, force=True)
...
def process_response(response: Response):
	...
	if hasattr(nts.local, "rate_limiter"):
		response.headers.update(nts.local.rate_limiter.headers())

	if trace_id := nts.monitor.get_trace_id():
		response.headers.update({"X-nts-Request-Id": trace_id})
	...
	response.headers.update(nts.local.response_headers)
	...
	if hasattr(nts.local, "cookie_manager") and not public_cache:
...
def set_cors_headers(response):
	...
	if hasattr(nts.local, "allow_cors"):
		allowed_origins = nts.local.allow_cors

	if not (
		allowed_origins and (request := nts.local.request) and (origin := request.headers.get("Origin"))
	):
		return
...
def make_form_dict(request: Request):
	...
	if request_data and request.is_json:
		try:
			...
			nts.throw(_("Invalid request body"), nts.DataError)
	else:
		args = {}
...
def handle_exception(e):
	...
	respond_as_json = (
		nts.get_request_header("Accept") and (nts.local.is_ajax or "application/json" in accept_header)
	...
	if http_status_code >= 500 or nts.conf.developer_mode:
...
def sync_database():
	db = getattr(nts.local, "db", None)
	if not db:
		# db isn't initialized, can't commit or rollback
		return

	# if HTTP method would change server state, commit if necessary
	if nts.local.request.method in UNSAFE_HTTP_METHODS or nts.local.flags.commit:
	...
	if session := getattr(nts.local, "session_obj", None):
		nts.request.after_response.add(session.update)
