...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		...
		from nts.automation.doctype.auto_repeat_user.auto_repeat_user import AutoRepeatUser
	...
	def validate_reference_doctype(self):
		if nts.in_test or nts.flags.in_patch:
			return
	...
	def validate_dates(self):
		...
		if self.end_date:
			...
			if end_date == getdate():
				nts.throw(_("End Date cannot be today."))
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not nts.in_test:
...
def generate_message_preview(name: str):
	...
	auto_repeat = nts.get_doc("Auto Repeat", str(name))
	doc = nts.get_doc(auto_repeat.reference_doctype, auto_repeat.reference_document)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = nts.render_template(auto_repeat.message, {"doc": doc})
	if auto_repeat.subject:
		subject_preview = nts.render_template(auto_repeat.subject, {"doc": doc})
