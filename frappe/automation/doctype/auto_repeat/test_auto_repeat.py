...
class TestAutoRepeat(IntegrationTestCase):
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = nts.get_doc(doctype=doctype, test="test submit on creation").insert()
	...
	def test_auto_repeat_assignee_with_separate_documents(self):
		...
		new_todo_count = nts.db.count("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")
