...
from nts.utils import add_to_date, execute_in_shell, get_bench_path, get_bench_relative_path, now
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute nts.lacol.site")
		self.assertEqual(self.returncode, 1)
		self.assertIsNotNone(self.stderr)

		# test 4: execute a command with kwargs
		self.execute(
			"bench --site {site} execute nts.bold --kwargs '{put_here}'",
			{"put_here": '{"text": "DocType"}'},  # avoid escaping errors
		)
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout, nts.bold(text="DocType"))

	@skipIf(
		nts.conf.db_type == "sqlite",
	...
	def test_restore(self):
		...
		global_config = {
			...
			"root_password": nts.conf.mariadb_root_password or nts.conf.root_password,
			"db_type": nts.conf.db_type,
		}
	...
	@skipIf(
		not (nts.conf.root_password and nts.conf.admin_password and nts.conf.db_type != "sqlite"),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--db-root-username {nts.conf.root_login} "
			f"--db-root-password {nts.conf.root_password} "
	...
	def test_set_global_conf(self):
		key = "answer"
		value = nts.generate_hash()
		_ = nts.get_site_config()
		self.execute(f"bench set-config {key} {value} -g")
	...
	def test_different_db_username(self):
		...
		kwargs = {
			...
			"db_root_username": nts.conf.root_login,
			"db_root_password": nts.conf.root_password or "",
		}
		self.execute(
	...
	def test_existing_db_username(self):
		...
		if nts.conf.db_type == "mariadb":
			from nts.database.mariadb.setup_db import get_root_connection

			root_conn = get_root_connection()
			root_conn.sql(f"CREATE USER '{user}'@'localhost'")
		else:
			from nts.database.postgres.setup_db import get_root_connection
...
class TestBackups(BaseTestCommands):
	...
	def test_clear_log_table(self):
		...
		nts.db.sql_ddl(IntegrationTestCase.normalize_sql("DROP TABLE IF EXISTS `tabError Log temp_table`"))
