...
if TYPE_CHECKING:
	from nts.testing import TestRunner
...
def main(
	...
	if lightmode:
		from nts.testing.config import TestParameters
	...
	from nts.testing.environment import _cleanup_after_tests, _initialize_test_environment
	...
	testing_module_logger = logging.getLogger("nts.testing")
...
def run_tests_in_light_mode(test_params):
	from nts.testing.loader import ntsTestLoader
	from nts.testing.result import ntsTestResult
	from nts.tests.utils import toggle_test_mode

	# init environment
	nts.init(test_params.site)
	if not nts.db:
		nts.connect()
	...
	suite = ntsTestLoader().discover_tests(test_params)
	result = unittest.TextTestRunner(failfast=test_params.failfast, resultclass=ntsTestResult).run(suite)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(nts.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from nts.testing import discover_doctype_tests
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		nts.init(site)
		allow_tests = nts.conf.allow_tests
...
def run_ui_tests(
	...
	admin_password = nts.get_conf().admin_password
	...
	if not (
		...
		package_json_path = nts.get_app_source_path("nts", "package.json")
