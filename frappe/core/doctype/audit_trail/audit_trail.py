...
class AuditTrail(Document):
	...
	def validate_fields(self):
		...
		for field in fields_dict:
			if not fields_dict[field]:
				nts.throw(_("{} field cannot be empty.").format(nts.bold(field)))

	def validate_document(self):
		if not nts.db.exists(self.doctype_name, self.document):
			nts.throw(
				_("The selected document {0} is not a {1}.").format(
					nts.bold(self.document), nts.bold(self.doctype_name)
				)
			)
	...
	def get_amended_documents(self):
		...
		creation = nts.db.get_value(self.doctype_name, self.document, "creation")
		while (
			curr_doc
			...
			creation = nts.db.get_value(self.doctype_name, curr_doc, "creation")
