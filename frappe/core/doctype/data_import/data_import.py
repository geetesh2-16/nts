...
from nts.utils.background_jobs import enqueue, get_redis_conn, is_job_enqueued
...
class DataImport(Document):
	...
	def set_delimiters_flag(self):
		if self.import_file:
			nts.flags.delimiter_options = self.delimiter_options or ","
	...
	def validate_doctype(self):
		...
		if not nts.has_permission(self.reference_doctype, "import"):
			nts.throw(
	...
	def start_import(self):
		...
		run_now = nts.in_test or nts.conf.developer_mode
...
def stop_data_import(doc_name: str):
	...
	data_import = nts.get_doc("Data Import", doc_name)
	data_import.check_permission("write")

	rq_job_id = f"{nts.local.site}||data_import||{doc_name}"
