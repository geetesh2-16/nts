...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: nts.Document = nts.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: nts.Document = nts.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameContent(IntegrationTestCase):
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = nts.get_doc("File", _file.name)
...
class TestPublicFileRestriction(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		if not nts.db.exists("User", "test_restricted@example.com"):
			user = nts.get_doc(
				{
					"doctype": "User",
	...
	def test_non_system_manager_cannot_upload_public_file_when_setting_enabled(self):
		...
		nts.set_user("test_restricted@example.com")
		...
		self.assertRaises(nts.PermissionError, file_doc.insert)
