...
import nts
from nts import _
from nts.defaults import clear_default, set_default
...
class Language(Document):
	...
	def on_update(self):
		...
		nts.client_cache.delete_value("languages")
	...
	def update_user_defaults(self):
		...
		should be updated.
		"""
		users = nts.get_all("User", filters={"language": self.name}, pluck="name")
...
def sync_languages():
	"""Create Language records from nts/geo/languages.csv"""
	from csv import DictReader

	with open(nts.get_app_path("nts", "geo", "languages.csv")) as f:
		reader = DictReader(f)
		for row in reader:
			if not nts.db.exists("Language", row["language_code"]):
				doc = nts.new_doc("Language")
				doc.update(row)
				doc.insert()
