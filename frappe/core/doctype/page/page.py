...
import nts
from nts import _, conf, get_module_path, safe_decode
...
class Page(Document):
	...
	def on_trash(self):
		...
		nts.db.after_commit(self.delete_folder_with_contents)

	def delete_folder_with_contents(self):
		try:
			module_path = get_module_path(self.module)
			dir_path = os.path.join(module_path, "page", nts.scrub(self.name))
