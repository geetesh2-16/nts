...
class TestScheduledJobType(IntegrationTestCase):
	...
	def test_throws_on_duplicate_job(self):
		job_config = dict(
			...
			method="nts.desk.notifications.clear_notifications",
			frequency="Weekly",
		)
		nts.get_doc(job_config).insert()

		duplicate_job = nts.get_doc(job_config)
	...
	def test_weekly_job(self):
		job = nts.get_doc(
			"Scheduled Job Type",
			dict(method="nts.desk.form.document_follow.send_weekly_updates"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
