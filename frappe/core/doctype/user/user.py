...
from nts.www.login import sanitize_redirect
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from nts.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from nts.core.doctype.user_session_display.user_session_display import UserSessionDisplay
	...
	def active_sessions(self):
		...
		for session in sessions_data:
			data = nts.parse_json(session.sessiondata)
			...
			session_docs.append(
				{
					...
					"is_current": session.sid == nts.session.sid,
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = nts.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	def on_update(self):
		...
		now = nts.in_test or nts.flags.in_install
		...
		if self.has_value_changed("language"):
			...
			if self.language:
				language = nts.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						nts.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					nts.defaults.clear_default(key, parent=self.name)
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		sender = (
			nts.session.user not in STANDARD_USERS and get_formatted_email(nts.session.user)
		) or None
	...
	def on_trash(self):
		...
		invites = nts.db.get_all("User Invitation", filters={"email": self.name}, pluck="name")
		for invite in invites:
			invite_doc = nts.get_doc("User Invitation", invite)
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(nts.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(nts.get_system_settings("allow_login_using_user_name"))
...
def get_all_roles():
	...
	roles = (
		...
		.select(Role.name)
		.where(
			(Role.name.notin(nts.permissions.AUTOMATIC_ROLES)) & (Role.disabled == 0) & domain_condition
		)
		.orderby(Role.name)
		.run(as_dict=True)
	)
...
def get_roles(arg=None):
	...
	return nts.get_roles(nts.form_dict.get("uid", nts.session.user))
...
def update_password(
	...
	logout_all_sessions = cint(logout_all_sessions) or nts.get_system_settings("logout_on_password_reset")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	...
	enable_password_policy = nts.get_system_settings("enable_password_policy")
	...
	if new_password:
		...
		minimum_password_score = cint(nts.get_system_settings("minimum_password_score"))
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				nts.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
...
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	...
	if user:
		...
		max_signups_allowed_per_hour = cint(nts.get_system_settings("max_signups_allowed_per_hour") or 300)
		users_created_past_hour = nts.db.get_creation_count("User", 60)
		...
		default_role = nts.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			nts.cache.hset("redirect_after_login", user.name, sanitize_redirect(redirect_to))
...
def impersonate(user: str, reason: str):
	nts.has_permission("User", "impersonate")
...
def clear_session(sid_hash: str):
	from nts.sessions import delete_session

	sessions = nts.qb.DocType("Sessions")
	sessions_data = (
		nts.qb.from_(sessions).select(sessions.sid).where(sessions.user == nts.session.user)
	).run(pluck=True)

	for session in sessions_data:
		if sha256_hash(session) == sid_hash:
			delete_session(sid=session, reason="Force Logged out by the user", user=nts.session.user)
			nts.toast(_("Successfully signed out"))
			return
