...
class Database:
	...
	def sql(
		...
		*,
		...
		if debug:
			time_end = time()
			nts.log(f"Execution time: {(time_end - time_start) * 1000:.3f} ms")
	...
	def explain_query(self, query, values=EmptyQueryValues):
		...
		try:
			...
			nts.log(json.dumps(results, indent=1))
	...
	def clear_db_table_cache(query_type: str):
		if query_type in CREATE_OR_DROP:
			nts.client_cache.delete_value("db_tables")
	...
	def get_single_value(
		...
		*,
		...
		df = nts.get_meta(doctype).get_field(fieldname) or get_default_df(fieldname)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if isinstance(dn, FilterValue):
			nts.clear_document_cache(dt, convert_to_value(dn))
		else:
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = nts.client_cache.get_value(key)
		if columns is None:
			...
			if columns:
				nts.cache.set_value(key, columns)
	...
	from nts.deprecation_dumpster import is_column_missing as _is_column_missing
	...
	def log_touched_tables(self, query, query_type):
		...
		touched_tables = nts.local.flags.touched_tables

		if touched_tables is None:
			touched_tables = set()
			nts.local.flags.touched_tables = touched_tables
	...
	def bulk_insert(
		...
		*,
		...
		if ignore_duplicates:
			...
			if nts.conf.db_type in ("mariadb", "sqlite"):
				query = query.ignore()
...
def get_print_sql_flag() -> bool:
	flag_value = nts.client_cache.get_value("flag_print_sql")
	if flag_value is None:
		flag_value = False
		nts.client_cache.set_value("flag_print_sql", flag_value)
