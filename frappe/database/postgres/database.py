...
class PostgresDatabase(PostgresExceptionUtil, Database):
	...
	@property
	def db_schema(self):
		return nts.conf.get("db_schema", "public").replace("'", "").replace('"', "")
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		if (columns := nts.client_cache.get_value(key)) is not None:
			return columns
		...
		nts.client_cache.set_value(key, columns)
	...
	def describe(self, doctype: str) -> list | tuple:
		...
		return self.sql(
			f"SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '{table_name}' and table_schema='{nts.conf.get('db_schema', 'public')}'"
		)
