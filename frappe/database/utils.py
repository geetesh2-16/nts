...
def get_doctype_sort_info(doctype: str) -> tuple[str, str]:
	...
	"""
	from nts.database.query import CORE_DOCTYPES
	...
	def run(self):
		...
		while self._functions:
			...
			try:
				...
				nts.log_error(title="Error executing commit_after_response callback")
...
def commit_after_response(func):
	...
	request = getattr(nts.local, "request", False)
	if not request or nts.in_test:
		func()
		return
...
def drop_index_if_exists(table: str, index: str):
	...
	try:
		if nts.db.db_type == "postgres":
			...
			nts.db.sql_ddl(f'DROP INDEX IF EXISTS "{safe_index}"')
		else:
