...
import nts
from nts import _
from nts.boot import get_sidebar_items
from nts.desk.desktop import get_workspace_sidebar_items, save_new_widget
...
class Workspace(Document):
	...
	def delete_from_my_workspaces(self):
		if not self.public:
			my_workspaces = nts.get_doc("Workspace Sidebar", f"My Workspaces-{nts.session.user}")
			for w in my_workspaces.items:
				if self.name == w.link_to:
					nts.delete_doc("Workspace Sidebar Item", w.name)
