...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	if cached:
		meta = nts.client_cache.get_value(key)
		if not meta:
		...
		meta = FormMeta(doctype, cached=nts.conf.developer_mode)
...
"""
	...
	def load_workspaces(self):
		Shortcut = nts.qb.DocType("Workspace Shortcut")
		Workspace = nts.qb.DocType("Workspace")
		shortcut = (
			nts.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = nts.qb.DocType("Workspace Link")
			link = (
				nts.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
