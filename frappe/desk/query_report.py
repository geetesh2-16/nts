...
from nts.permissions import get_role_permissions, get_roles, has_permission
...
def get_script(report_name):
	...
	if not script:
		script = "nts.query_reports['{}']={{}}".format(report_name)
...
def export_query():
	...
	if export_in_background:
		...
		nts.enqueue(
			"nts.desk.query_report.run_export_query_job",
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	ref_doctype_meta = nts.get_meta(ref_doctype)
		...
		if not matched_for_doctype:
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if match:
						match = has_unrestricted_read_access(doctype=ref_doctype, user=nts.session.user)
...
def has_unrestricted_read_access(doctype, user):
	...
	standard_perm_exists = nts.db.exists(
		"DocPerm",
		permission_filters,
	)

	custom_perm_exists = nts.db.exists(
		"Custom DocPerm",
		permission_filters,
	)
