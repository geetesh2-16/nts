...
from nts import _, bold, is_whitelisted, validate_and_sanitize_search_inputs
...
from nts.utils import cint, cstr, escape_html, unique
...
def validate_ignore_user_permissions(form_doctype, link_fieldname, link_doctype):
	def _throw(message):
		nts.throw(message, title=_('Error validating "Ignore User Permissions"'))

	meta = nts.get_meta(form_doctype)
	...
	if link_field.fieldtype == "Table MultiSelect":
		child_meta = nts.get_meta(link_field.options)
