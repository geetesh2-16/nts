...
def send_report_email(
	...
	file_url = nts.utils.get_url(_file.get_url())
	file_retention_hours = nts.get_system_settings("delete_background_exported_reports_after") or 48

	nts.sendmail(
		recipients=[user_email],
		subject=nts._("Your exported report: {0}").format(report_name),
		message=nts._(
...
def delete_old_exported_report_files():
	...
	cutoff = nts.utils.add_to_date(nts.utils.now_datetime(), hours=-file_retention_hours)
	old_files = nts.get_all(
		"File",
		filters={
			"attached_to_doctype": "Report",
			"creation": ("<", cutoff),
	...
	for file_name in old_files:
		try:
			nts.delete_doc("File", file_name)
		except Exception:
			nts.log_error(f"Failed to delete old report file {file_name}")
