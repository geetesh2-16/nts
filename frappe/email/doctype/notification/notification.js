			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (nts.model.table_fields.includes(df.fieldtype)) {
						let child_fields = nts.get_doc("DocType", df.options).fields;
...
• Amount: {{ doc.grand_total }}
		...
		if (template) {
			...
			if (frm.doc.channel === "Email") {
				nts.utils.highlight_pre(message_examples_field.$wrapper);
			}
		}
	},
};
...
nts.ui.form.on("Notification", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Preview"), () => {
				...
				let dialog = new nts.views.RenderPreviewer(args);
				return dialog;
			});
		}
	...
	set_up_filters_editor(frm) {
		...
		nts.model.with_doctype(frm.doc.document_type, () => {
			const filter_group = new nts.ui.FilterGroup({
				parent: parent,
				doctype: frm.doc.document_type,
				on_change: () => {
					nts.model.set_value(
