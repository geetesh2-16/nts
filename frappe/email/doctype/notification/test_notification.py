...
def get_test_doctype_with_attach_field(doctype_name="Test Attach Doctype"):
	...
	try:
		...
		if not nts.db.exists("DocType", doctype_name):
			nts.get_doc(
				{
					"doctype": "DocType",
					"name": doctype_name,
		...
		nts.db.delete(doctype_name)
		nts.delete_doc_if_exists("DocType", doctype_name, force=1)
		nts.db.commit()
...
def create_test_file(file_name="test_attachment.txt", content="Test file content"):
	...
	try:
		...
		nts.delete_doc_if_exists("File", file_doc.name, force=1)
		nts.db.commit()
...
class TestNotification(IntegrationTestCase):
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(nts.ValidationError, notification.insert)
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = nts.utils.add_days(user.birth_date, 1).date()
			user.save()
		...
		notification = nts.get_doc(
			{
				...
				"message": "{% set val = nts.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
	...
	def test_filters_condition(self):
		...
		nts.delete_doc_if_exists("Notification", "Test Filters Condition")
...
❯ bench run-tests --module nts.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		nts.db.delete("Event", {"name": self.event.name})
# 		nts.db.delete("Notification", {"name": self.notification["name"]})
# 		nts.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							nts.db.count("Notification Log", {"subject": n.subject}),
