...
import nts
from nts import _
from nts.ntsclient import ntsClient, ntsOAuth2Client
from nts.utils import convert_utc_to_system_timezone, get_datetime, get_system_timezone

CHUNK_SIZE = 5 * 1024 * 1024  # 5MB


class ntsMail:
	"""Class to interact with the nts Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> ntsClient | ntsOAuth2Client:
		"""Returns a ntsClient or ntsOAuth2Client instance."""

		if hasattr(nts.local, "nts_mail_clients"):
			if client := nts.local.nts_mail_clients.get(email):
				return client
		else:
			nts.local.nts_mail_clients = {}

		client = (
			ntsOAuth2Client(url=site, access_token=access_token)
			if access_token
			else ntsClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		nts.local.nts_mail_clients[email] = client
	...
	def request(
		...
		"""Makes a request to the nts Mail API."""
	...
	def send_raw(
		...
		"""Sends an email using the nts Mail API."""
	...
	def pull_raw(
		...
		"""Pull emails for the account using the nts Mail API."""
		...
		headers = {"X-Site": nts.utils.get_url()}
