...
class SocialLoginKey(Document):
	...
	def validate(self):
		...
		if self.auth_url_data:
			try:
				...
				nts.throw(_("Auth URL data should be valid JSON"))

		if self.api_endpoint_args:
			try:
				json.loads(self.api_endpoint_args)
			except json.JSONDecodeError:
				nts.throw(_("API Endpoint Args should be valid JSON"))
	...
	def get_social_login_provider(self, provider, initialize=False):
		...
		providers["Keycloak"] = {
			...
			"enable_social_login": 1,
			"custom_base_url": 1,
			"redirect_url": "/api/method/nts.integrations.oauth2_logins.login_via_keycloak",
