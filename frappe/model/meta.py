...
from nts.utils import cached_property, cast, cint, cstr
...
def get_meta(doctype: "str | DocType", cached: bool = True) -> "_Meta":
	...
	if (
		...
		and (meta := nts.client_cache.get_value(f"doctype_meta::{doctype}"))
	):
		return meta
	...
	nts.client_cache.set_value(key, meta)
	return meta
...
def clear_meta_cache(doctype: str = "*"):
	...
	if doctype == "*":
		nts.client_cache.delete_keys(key)
	else:
		nts.client_cache.delete_value(key)
...
class Meta(Document):
	...
	def get_masked_fields(self):
		...
		if nts.session.user == "Administrator":
			return []
		cache_key = f"masked_fields::{self.name}::{nts.session.user}"
		masked_fields = nts.cache.get_value(cache_key)
		...
		if masked_fields is None:
			...
			nts.cache.set_value(cache_key, masked_fields)
	...
	def _valid_fields(self):
		if (nts.flags.in_install or nts.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if nts.share.get_shared(self.name, user, rights=["read"], limit=1):
...
def get_precision_from_currency_format(currency: str) -> int:
	...
	from nts.locale import get_number_format
	from nts.utils.number_format import NumberFormat

	use_format_from_currency = nts.get_system_settings("use_number_format_from_currency")
	number_format = get_number_format()
	if use_format_from_currency:
		currency_format = nts.db.get_value("Currency", currency, "number_format", cache=True)
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			...
			return nts._dict(fieldname=fieldname, fieldtype="Link", options="User")
