...
def has_permission(
	...
	*,
	...
	:param print_logs: If True, will display a message using nts.msgprint
	...
	if doc:
		if isinstance(doc, str | int):
			...
			doc = nts.get_lazy_doc(meta.name, doc)
...
def has_user_permission(doc, user=None, debug=False, *, ptype=None):
	...
	if (
		...
		and (not docname or (docname and not nts.db.exists(doctype, docname, cache=True)))
	):
...
def remove_user_permission(doctype, name, user):
	...
	nts.delete_doc("User Permission", user_permission_name, force=True)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	for d in user_permissions_for_doctype:
		nts.delete_doc("User Permission", d.name, force=True)
...
def update_permission_property(
	...
	from nts.core.doctype.custom_docperm.custom_docperm import update_custom_docperm
	from nts.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	custom_docperm = nts.db.get_value(
...
def reset_perms(doctype):
	...
	for custom_docperm in nts.get_all("Custom DocPerm", filters={"parent": doctype}, pluck="name"):
		nts.delete_doc("Custom DocPerm", custom_docperm, ignore_permissions=True, force=True)
