...
nts.ui.form.ControlLink = class ControlLink extends nts.ui.form.ControlData {
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			nts.utils.add_link_title(this.get_options(), value, label);
		}
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				let _label = nts.utils.escape_html(me.get_translated(d.label));
				...
				if (
					...
					html +=
						'<br><span class="small">' +
						__(nts.utils.escape_html(d.description)) +
						"</span>";
				}
		...
		this._debounced_input_handler = nts.utils.debounce(this.on_input.bind(this), 500);
			...
			callback: async (r) => {
				...
				if (!this.df.only_select) {
					...
					let custom__link_options =
						...
						nts.ui.form.ControlLink.link_options(this);
	...
	async get_filter_description(filters) {
		...
		function formatValueForDisplay(docfield, val) {
			...
			// Numbers, dates, etc. - not translated, not quoted
			return nts.format(val, docfield || {}, { inline: true });
		}
		...
		async function describe_filter(filter) {
			...
			await nts.model.with_doctype(_doctype, () => {});

			const docfield = nts.meta.get_docfield(_doctype, fieldname);
			const label = docfield ? docfield.label : nts.model.unscrub(fieldname);
		...
		const filter_string = nts.utils.comma_and(descriptions);
	...
	validate_link_and_fetch(value) {
		...
		return nts
			.xcall(
				"nts.client.validate_link_and_fetch",
				{
					...args,
					docname: value,
