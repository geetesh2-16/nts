	...
	get_timeline_item(item) {
		...
		if (!item.hide_timestamp && !item.is_card) {
			timeline_content.append(
				`<span> · ${
					cint(nts.boot.user.show_absolute_datetime_in_timeline) ||
					cint(nts.boot.sysdefaults.show_absolute_datetime_in_timeline)
						? nts.datetime.str_to_user(item.creation)
...
nts.ui.BaseTimeline = BaseTimeline;
