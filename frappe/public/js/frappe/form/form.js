...
nts.ui.form.Form = class ntsForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = nts.get_meta(doctype_layout_name);
	...
	setup_notify_on_rename() {
		...
		nts.realtime.on("doc_rename", (data) => {
			...
			if (data.doctype == this.doctype && data.old == this.docname) {
				...
				nts.set_route("Form", this.doctype, data.new);
			}
		});
	}
	...
	discard(btn, callback, on_error) {
		...
		return new Promise((resolve) => {
			nts.confirm(__("Discard {0}", [this.docname]), function () {
	...
	_discard(btn, on_error, skip_confirm) {
		...
		const discard_doc = () => {
			...
			me.script_manager.trigger("before_discard").then(() => {
				...
				//nts.ui.form.discard(me, after_discard, btn);
				nts.call({
					freeze: true,
					method: "nts.desk.form.save.discard",
					args: {
						doctype: me.doc.doctype,
						name: me.doc.name,
					},
	...
	navigate_records(prev) {
		...
		nts
			.call({ method: "nts.desk.form.utils.get_next", args, freeze: true })
			.then((r) => {
				if (r.message) {
	...
	email_doc(message) {
		return new nts.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	get_title() {
		return nts.model.get_doc_title(this.doc);
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		nts.meta.get_table_fields(this.doctype, true).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		if (nts.meta.docfield_map[this.doctype][fieldname]) {
			...
			nts.meta.get_table_fields(this.doctype, true).every(function (df) {
			...
			if (value) {
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/desk/${nts.router.slug(df.options)}/${escaped_name}"
