...
nts.ui.form.ScriptManager = class ScriptManager {
	...
	has_handler(event_name) {
		...
		return (
			nts.ui.form.handlers[this.frm.doctype] &&
			nts.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (nts.ui.form.handlers["*"] && nts.ui.form.handlers["*"][event_name]) {
			$.each(nts.ui.form.handlers["*"][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
