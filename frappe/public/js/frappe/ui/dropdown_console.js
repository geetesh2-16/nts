export class DropdownConsole {
	constructor() {
		this.dialog = new nts.ui.Dialog({
			...
			fields: [
				{
					description: `
					${nts.utils.icon("solid-warning", "xs")}
	...
	async load_contextual_boilerplate() {
		...
		if (cur_frm && !cur_frm.is_new()) {
			default_code = `doc = nts.get_doc("${cur_frm.doc.doctype}", "${cur_frm.doc.name}")\n`;
		} else if (cur_list && nts.get_route()[0] == "List") {
			let args = cur_list.get_args();
			default_code = `docs = nts.get_all("${args.doctype}",
				fields="*",
	...
	async execute_code() {
		...
		const start = nts.datetime.now_datetime(true);
		let { output } = await nts.xcall(
			"nts.desk.doctype.system_console.system_console.execute_code",
			{
				doc: {
		...
		const end = nts.datetime.now_datetime(true);
		...
		output_field.set_description(`Executed in ${time_taken} milliseconds.
			<a target="_blank" href="/desk/console-log?owner=${nts.session.user}" >View Logs</a>`);
	}

	async load_completions() {
		let me = this;
		let items = await nts.xcall(
			"nts.core.doctype.server_script.server_script.get_autocompletion_items",
			null,
			"GET",
			{ cache: true }
		);
