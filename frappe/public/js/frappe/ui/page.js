...
nts.ui.Page = class Page {
	...
	setup_mobile_awesomebar() {
		if (nts.boot.desk_settings.search_bar && nts.is_mobile()) {
		...
		$(".main-section").scroll(
			nts.utils.throttle(() => {
				...
				if (
					...
					(nts.boot.read_only || nts.boot.user.impersonated_by)
				) {
					$(".page-head").css("top", "-15px");
				} else if (nts.boot.read_only || nts.boot.user.impersonated_by) {
	...
	setup_main_sidebar_toggle() {
		$(".sidebar-toggle-btn.navbar-brand").on("click", (event) => {
			nts.app.sidebar.set_height();
			nts.app.sidebar.toggle_width();
			nts.app.sidebar.prevent_scroll();
		});
	}
