nts.provide("nts.ui.sidebar_item");
nts.ui.sidebar_item.TypeLink = class SidebarItem {
	constructor(opts) {
		this.item = opts.item;
	...
	get_path() {
		...
		if (this.item.type === "Link") {
			if (this.item.link_type === "Report") {
				...
				if (!nts.app.sidebar.editor.edit_mode) {
					if (this.item.report) {
				...
				path = nts.utils.generate_route(args);
			} else if (this.item.link_type == "Workspace") {
				let workspaces = nts.workspaces[nts.router.slug(this.item.link_to)];
				if (workspaces.public) {
					path = "/desk/" + nts.router.slug(this.item.link_to);
				} else {
					path = "/desk/private/" + nts.router.slug(workspaces.title);
				}
				...
				if (this.item.filters) {
					let filters_json = JSON.parse(
						nts.utils.get_filter_as_json(JSON.parse(this.item.filters))
					);
	...
	make() {
		...
		this.wrapper = $(
			nts.render_template("sidebar_item", {
				item: this.item,
				path: this.path,
				edit_mode: nts.app.sidebar.editor.edit_mode,
			})
		);
		...
		let menu_items = [
			{
				...
				icon: "pen",
				onClick: () => {
					nts.app.sidebar.editor.perform_action("edit", me.item);
				},
			},
			{
				...
				icon: "add",
				onClick: () => {
					nts.app.sidebar.editor.perform_action("add_below", me.item);
				},
			},
			{
				label: "Duplicate",
				icon: "copy",
				onClick: () => {
					nts.app.sidebar.editor.perform_action("duplicate", me.item);
				},
			},
			{
				label: "Delete",
				...
				onClick: () => {
					console.log(me.item);
					nts.app.sidebar.editor.perform_action("delete", me.item);
				},
			},
		];
		return menu_items;
...
nts.ui.sidebar_item.TypeSectionBreak = class SectionBreakSidebarItem extends (
	nts.ui.sidebar_item.TypeLink
) {
	prepare(opts) {
	...
	add_items() {
		...
		this.nested_items.forEach((f) => {
			this.items.push(
				nts.app.sidebar.make_sidebar_item({
	...
	enable_collapsible(item, $item_container) {
		...
		if (item.collapsible) {
			...
			this.$drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
				.html(nts.utils.icon(drop_icon, "sm", "", "", "", "", stroke_color))
	...
	setup_event_listner() {
		...
		$(this.wrapper.find(".standard-sidebar-item")[0]).on("click", (e) => {
			...
			if (!nts.app.sidebar.sidebar_expanded) {
				nts.app.sidebar.open();
				this.open();
			}
		});
	}
	...
	get_menu_items() {
		...
		let menu_items = [
			...
			},
			{
				...
				icon: "add",
				onClick: () => {
					nts.app.sidebar.editor.show_new_dialog({
						nested: true,
						parent_item: me.item,
					});
				},
...
nts.ui.sidebar_item.TypeSpacer = class SpacerItem extends nts.ui.sidebar_item.TypeLink {
	constructor(item, items) {
		super(item);
	}
};

nts.ui.sidebar_item.TypeSidebarItemGroup = class SpacerItem extends (
	nts.ui.sidebar_item.TypeLink
) {
	constructor(item, items) {
		super(item);
		this.title = nts.app.sidebar.workspace_title;
		this.setup_click();
	}
	...
	setup_click() {
		...
		this.wrapper.on("click", function () {
			nts.call({
				method: "nts.desk.doctype.sidebar_item_group.sidebar_item_group.get_reports",
				args: { module_name: nts.app.sidebar.workspace_title },
				callback: function (r) {
					if (r.message) {
						let links_html = "";
...
nts.ui.sidebar_item.TypeButton = class SidebarButton extends nts.ui.sidebar_item.TypeLink {
	constructor(item) {
		super(item);
		this.title = nts.app.sidebar.workspace_title;
