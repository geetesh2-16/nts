...
nts.views.FileView = class FileView extends nts.views.ListView {
	...
	prepare_datum(d) {
		...
		title = nts.utils.escape_html(title);
	...
	render_grid_view() {
		...
		let html = this.data
			.map((d) => {
				...
				const file_url = nts.utils.escape_html(d.file_url);
	...
	setup_copy_event() {
		this.$result.on("click", ".copy-file-url", (e) => {
			nts.utils.copy_to_clipboard(e.currentTarget.getAttribute("data-file-url"));
			e.preventDefault();
			e.stopPropagation();
		});
	}
