	...
	show() {
		nts.views.KanbanView.get_kanbans(this.doctype).then((kanbans) => {
			if (!kanbans.length) {
				...
				const last_board = nts.get_user_settings(this.doctype)["Kanban"]
					?.last_kanban_board;
				if (last_board && kanbans.includes(last_board)) {
					nts.set_route("List", this.doctype, "Kanban", last_board);
					return;
				} else {
					const first_board = kanbans[0];
					nts.set_route("List", this.doctype, "Kanban", first_board.name);
					return;
				}
			} else {
	...
	return get_kanban_boards().then((kanban_boards) => {
		if (kanban_boards) {
			kanban_boards.forEach((board) => {
				let route = `/desk/${nts.router.slug(board.reference_doctype)}/view/kanban/${
					board.name
				}`;
