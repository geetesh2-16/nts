...
nts.views.MapView = class MapView extends nts.views.ListView {
	...
	setup_view() {
		...
		this.streetLayer = L.tileLayer(
			nts.utils.map_defaults.tiles.default_tile.url,
			nts.utils.map_defaults.tiles.default_tile.options
		);
		this.satelliteLayer = L.tileLayer(
			nts.utils.map_defaults.tiles.satellite_tile.url,
			nts.utils.map_defaults.tiles.satellite_tile.options
		);
		this.labelsLayer = L.tileLayer(
			nts.utils.map_defaults.tiles.labels_tail.url,
			nts.utils.map_defaults.tiles.labels_tail.options
		);
		this.terrainLayer = L.tileLayer(
			nts.utils.map_defaults.tiles.terrain_lines_tail.url,
			nts.utils.map_defaults.tiles.terrain_lines_tail.options
		);
	...
	get_popup_content(feature) {
		return nts.utils.get_form_link(this.doctype, feature.properties.name, true);
	}
			...
			if (e.name === "Satellite") {
				...
				Object.values(this.map._layers).forEach((layer) => {
					if (
						...
						(layer._url === nts.utils.map_defaults.tiles.labels_tail.url ||
							layer._url === nts.utils.map_defaults.tiles.terrain_lines_tail.url)
					) {
