...
nts.views.Workspace = class Workspace {
	constructor(wrapper) {
		...
		this.workspaces = nts.boot.workspaces.pages;
		...
		this.sidebar = nts.app.sidebar;
		this.cached_pages = $.extend(true, {}, nts.boot.workspaces);
		this.has_access = nts.boot.workspaces.has_access;
		this.has_create_access = nts.boot.workspaces.has_create_access;
		this.setup();
		this.show();
	...
	get_pages() {
		return nts.xcall("nts.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	get_data(page) {
		return nts
			.call({
				method: "nts.desk.desktop.get_desktop_page",
				args: {
	...
	get_page_to_show() {
		...
		if (nts.boot.user.default_workspace) {
			default_page = {
				name: nts.boot.user.default_workspace.name,
	...
	async show_page(page) {
		...
		if (this.workspaces.length) {
			...
			if (nts.is_mobile()) {
				nts.breadcrumbs.add({
					type: "Custom",
					label: header_dropdown + `${nts.utils.icon("chevron-down")}`,
					route: "#",
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = nts.boot.module_app[nts.router.slug(this._page.module)];
				}
			...
			this.wrapper
				.find(".workspace-icon")
				.html(nts.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			nts.boot.workspaces = this.cached_pages;
			this.reload();
			...
			nts.call({
				...
				callback: (r) => {
					if (r.message) {
						...
						if (r.message) {
							nts.boot.workspaces = r.message.workspace_pages;
							me.workspaces = nts.boot.workspaces.pages;
							me.setup_pages(nts.boot.workspaces.pages);
							nts.boot.workspace_sidebar_item = r.message.sidebar_items;
						}

						if (new_page.public === 0) {
							nts.app.sidebar.setup("private");
						}
		...
		if (all_pages) {
			...
			for (let page of all_pages) {
				...
				if (!page.app && page.module) {
					page.app = nts.boot.module_app[nts.slug(page.module)];
				}
				nts.workspace_map[page.name] = page;
				nts.workspace_list.push(page);
			}
		}
	}
				...
				nts.call({
					...
					callback: function (res) {
						if (res.message) {
							...
							if (page.public) {
								nts.set_route("desk", page.title.toLowerCase());
							} else {
								nts.set_route("desk", "private", page.title.toLowerCase());
							}
						}
					},
				});
	...
	reload() {
		...
		return this.get_pages().then((r) => {
			nts.boot.workspaces = r;
			this.setup_pages(nts.boot.workspaces.pages);
			this.show();
