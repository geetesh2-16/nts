...
This README provides an overview of the test case framework available in nts. These utilities are designed to facilitate efficient and effective testing of nts applications.

This is different from the `nts.testing` module which houses the discovery and runner infrastructure for CLI and CI.
...
5. Specific test modules for various nts components

### Test Case Classes

#### UnitTestCase ([`classes/unit_test_case.py`](./classes/unit_test_case.py))

###### Import convention: `from nts.tests import UnitTestCase`

This class extends `unittest.TestCase` and provides additional utilities specific to the nts framework. It's designed for testing individual components or functions in isolation.

Key features include:
- Custom assertions for nts-specific comparisons
...
###### Import convention: `from nts.tests import IntegrationTestCase`
...
Various test modules (e.g., test_api.py, test_document.py) contain tests for specific nts core components and functionalities.

Note that Document tests are collocated alongside each Document module.

## Usage

To use these test utilities in your nts application tests, you can inherit from the appropriate test case class:

```python
from nts.tests import UnitTestCase
