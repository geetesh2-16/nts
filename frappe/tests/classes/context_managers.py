...
if TYPE_CHECKING:
	from nts.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from nts.utils.data import get_datetime, get_system_timezone
...
def enable_safe_exec() -> None:
	...
	conf = os.path.join(nts.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from nts.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly nts.tests.* namespace
