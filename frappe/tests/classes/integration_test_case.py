...
import nts
from nts.database.utils import get_query_type
...
class IntegrationTestCase(UnitTestCase):
	"""Integration test class for nts tests.
	...
	def setUpClass(cls) -> None:
		...
		if cls.doctype and cls.doctype not in nts.local.test_objects:
		...
		nts.db.commit()
		cls.globalTestRecords = MappingProxyType(nts.local.test_objects)
		if cls.SHOW_TRANSACTION_COMMIT_WARNINGS:
			nts.db.before_commit.add(_commit_watcher)

		# enqueue teardown actions (executed in LIFO order)
		cls.addClassCleanup(_restore_ctx_locals, copy.deepcopy(nts.local.flags))
...
def _restore_ctx_locals(flags):
	...
	nts.local.response = nts._dict({"docs": []})
	nts.local.cache = {}
	nts.local.lang = "en"
