...
class TestRedisWrapper(ntsAPITestCase):
	...
	def test_user_cache_clear(self):
		from nts.cache_manager import user_cache_keys

		# Set some keys that a user's cache would usually have
		user1 = nts.utils.random_string(10)
		user2 = nts.utils.random_string(10)
		for key in user_cache_keys:
			nts.cache.hset(key, user1, key)
			nts.cache.hset(key, user2, key)

		nts.clear_cache(user=user1)

		# Check that the keys for user1 are gone
		for key in set(user_cache_keys) - {"home_page"}:
			self.assertFalse(nts.cache.hexists(key, user1))
			self.assertTrue(nts.cache.hexists(key, user2))

	def test_doctype_cache_clear(self):
		from nts.cache_manager import doctype_cache_keys

		# Set some keys that a user's cache would usually have
		doctype1 = new_doctype(nts.utils.random_string(10))
		doctype2 = new_doctype(nts.utils.random_string(10))
		for key in doctype_cache_keys:
			nts.cache.hset(key, doctype1.name, key)
			nts.cache.hset(key, doctype2.name, key)

		nts.clear_cache(doctype=doctype1.name)

		# Check that the keys for doctype1 are gone
		for key in doctype_cache_keys:
			self.assertFalse(nts.cache.hexists(key, doctype1.name))
			self.assertTrue(nts.cache.hexists(key, doctype2.name))

	def test_backward_compat_cache(self):
		self.assertEqual(nts.cache, nts.cache())


class TestHttpCache(ntsAPITestCase):
	def test_http_headers(self):
		resp = self.get(
			self.method("nts.client.is_document_amended"),
