...
class TestHooks(IntegrationTestCase):
	...
	def test_fixture_prefix(self):
		...
		app = "nts"
		if os.path.isdir(nts.get_app_path(app, "fixtures")):
			shutil.rmtree(nts.get_app_path(app, "fixtures"))
		...
		# every call to nts.get_hooks loads the hooks module into cache
		...
		if nts._load_app_hooks in nts.local.request_cache.keys():
			del nts.local.request_cache[nts._load_app_hooks]
		self.assertEqual([False], nts.get_hooks("fixture_auto_order", app_name=app))
		self.assertEqual(
			[
				{"dt": "User"},
				{"dt": "Contact"},
			...
			nts.get_hooks("fixtures", app_name=app),
		)
		...
		self.assertCountEqual(
			["user.json", "contact.json", "role.json"], os.listdir(nts.get_app_path(app, "fixtures"))
		)

		hooks.fixture_auto_order = True
		del nts.local.request_cache[nts._load_app_hooks]
		self.assertEqual([True], nts.get_hooks("fixture_auto_order", app_name=app))

		shutil.rmtree(nts.get_app_path(app, "fixtures"))
		export_fixtures(app)
		self.assertCountEqual(
			["1_user.json", "2_contact.json", "3_role.json"],
			os.listdir(nts.get_app_path(app, "fixtures")),
		)
...
class TestAPIHooks(ntsAPITestCase):
	def test_auth_hook(self):
		with self.patch_hooks({"auth_hooks": ["nts.tests.test_hooks.custom_auth"]}):
...
def custom_auth():
	_auth_type, token = nts.get_request_header("Authorization", "Bearer ").split(" ")
