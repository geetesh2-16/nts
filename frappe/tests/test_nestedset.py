...
from nts.desk.treeview import get_children
...
class TestNestedSet(IntegrationTestCase):
	...
	def test_desc_filters(self):
		...
		# db_query
		self.assertNotIn(record, nts.get_all(TEST_DOCTYPE, exclusive_filter, run=0).get_sql())
		self.assertIn(record, nts.get_all(TEST_DOCTYPE, inclusive_filter, run=0).get_sql())
		self.assertNotIn(record, nts.get_all(linked_doctype, exclusive_link, run=0).get_sql())
		self.assertIn(record, nts.get_all(linked_doctype, inclusive_link, run=0).get_sql())
