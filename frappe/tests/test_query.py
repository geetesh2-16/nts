...
from nts.tests.classes.context_managers import enable_safe_exec
from nts.tests.test_db_query import (
...
class TestQuery(IntegrationTestCase):
	...
	def test_multiple_tables_in_filters(self):
		...
		query = query.replace("LIKE", "ILIKE" if nts.db.db_type == "postgres" else "LIKE")
	...
	def test_string_fields(self):
		...
		self.assertEqual(
			nts.qb.get_query("User", fields=[Count("*")]).get_sql(),
	...
	def test_field_validation_select(self):
		...
		for field in valid_fields:
			try:
				nts.qb.get_query("User", fields=field).get_sql()
		...
		for field in invalid_fields:
			with self.assertRaises(
				(nts.PermissionError, nts.ValidationError),
	...
	def test_field_validation_filters(self):
		...
		for field in valid_fields:
			try:
				# Test in dict filter
				nts.qb.get_query("User", filters={field: "value"}).get_sql()
				# Test in list filter
				nts.qb.get_query("User", filters=[[field, "=", "value"]]).get_sql()
				# Test in list filter with doctype
				nts.qb.get_query("User", filters=[["User", field, "=", "value"]]).get_sql()
			except Exception as e:
				self.fail(f"Valid filter field '{field}' failed validation: {e}")

		for field in invalid_fields:
			with self.assertRaises(
				nts.ValidationError, msg=f"Invalid filter field '{{{field}: val}}' passed validation"
			):
				nts.qb.get_query("User", filters={field: "value"}).get_sql()
	...
	def test_field_validation_group_by(self):
		...
		for group_by_str in valid_fields:
			try:
				nts.qb.get_query("User", group_by=group_by_str).get_sql()
			except Exception as e:
	...
	def test_field_validation_order_by(self):
		...
		for order_by_str in valid_fields:
			try:
				nts.qb.get_query("User", order_by=order_by_str).get_sql()
			except Exception as e:
				self.fail(f"Valid ORDER BY string '{order_by_str}' failed validation: {e}")

		for order_by_str in invalid_fields:
			with self.assertRaises(
				(nts.PermissionError, ValueError, nts.ValidationError),
	...
	def test_filters(self):
		self.assertQueryEqual(
			...
			"SELECT `tabDocType`.`name` FROM `tabDocType` LEFT JOIN `tabModule Def` ON `tabModule Def`.`name`=`tabDocType`.`module` WHERE `tabModule Def`.`app_name`='nts'",
		)
	...
	def test_or_filters(self):
		...
		query = query = query.replace("LIKE", "ILIKE" if nts.db.db_type == "postgres" else "LIKE")
		self.assertQueryEqual(
			nts.qb.get_query(
				"DocType",
				fields=["name"],
	...
	def test_nested_filters(self):
		...
		self.assertEqual(nts.qb.get_query("User", filters=filters_and).get_sql(), expected_sql_and)
		...
		self.assertEqual(nts.qb.get_query("User", filters=filters_or).get_sql(), expected_sql_or)
		...
		self.assertEqual(nts.qb.get_query("User", filters=filters_mixed).get_sql(), expected_sql_mixed)
		...
		self.assertEqual(nts.qb.get_query("User", filters=filters_nested).get_sql(), expected_sql_nested)
		...
		self.assertEqual(
			nts.qb.get_query("User", filters=filters_single_group).get_sql(), expected_sql_single_group
		)
	...
	def test_invalid_nested_filters(self):
		...
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", filters=["email", "and", ["enabled", "=", 1]]).get_sql()
		self.assertIn("Invalid start for filter condition", str(cm.exception))
	...
	def test_build_match_conditions(self):
		...
		query = nts.qb.get_query("Test Blog Post", ignore_permissions=False)
		...
		query = str(nts.qb.get_query("Test Blog Post", ignore_permissions=False))
	...
	def test_ignore_permissions_for_query(self):
		...
		with self.assertRaises(nts.PermissionError):
			nts.qb.get_query("DocType", filters={"istable": 1}, ignore_permissions=False)

		result = nts.qb.get_query("DocType", filters={"istable": 1}, ignore_permissions=True).run()
	...
	def test_child_table_access_with_select_permission(self):
		...
		# Cleanup
		nts.set_user("Administrator")
		test_user = nts.get_doc("User", test_user_email)
		test_user.remove_roles(test_role)
		nts.delete_doc("Role", test_role, ignore_missing=True, force=True)
		nts.delete_doc("Note", {"title": test_note_title}, ignore_missing=True, force=True)

		# Setup Role with 'select' on Note and 'read' on Note Seen By
		nts.get_doc({"doctype": "Role", "role_name": test_role}).insert(ignore_if_duplicate=True)
		...
		test_user.remove_roles(test_role)
		nts.delete_doc("Role", test_role, force=True)
	...
	def test_nested_permission(self):
		...
		with self.set_user("test2@example.com"):
			data = nts.qb.get_query("Nested DocType", ignore_permissions=False).run(as_dict=1)
	...
	def test_is_set_is_not_set(self):
		...
		result = nts.qb.get_query("DocType", filters={"autoname": ["is", "not set"]}).run(as_dict=1)
		...
		result = nts.qb.get_query("DocType", filters={"autoname": ["is", "set"]}).run(as_dict=1)
	...
	def test_permission_query_condition(self):
		...
		from nts.desk.doctype.dashboard_settings.dashboard_settings import create_dashboard_settings

		# Create a Dashboard Settings for test user
		self.doctype = "Dashboard Settings"
		self.user = "test@example.com"

		original_hooks = nts.get_hooks("permission_query_conditions") or {}
		...
		query = nts.qb.get_query("Dashboard Settings", user=self.user, ignore_permissions=False)
		...
		nts.clear_cache()
		nts.hooks.permission_query_conditions = {}  # Clear hooks to test server script alone
		...
		script.delete()
		nts.clear_cache()
		nts.hooks.permission_query_conditions = original_hooks
	...
	def test_link_field_target_permission(self):
		...
		nts.set_user("Administrator")
		nts.delete_doc("DocType", target_dt_name, ignore_missing=True, force=True)
		nts.delete_doc("DocType", source_dt_name, ignore_missing=True, force=True)
		nts.delete_doc("Role", test_role, ignore_missing=True, force=True)
		test_user_doc = nts.get_doc("User", test_user)
		...
		# Create Records
		target_doc = nts.get_doc(
			doctype=target_dt_name, target_field="Secret Data", other_target_field="Public Data"
		).insert(ignore_permissions=True)
		source_doc = nts.get_doc(doctype=source_dt_name, link_field=target_doc.name).insert(
		...
		result_restricted = nts.qb.get_query(
			source_dt_name,
	...
	def test_filter_linked_field_permission(self):
		...
		with setup_test_user(set_user=True) as user:
			...
			nts.delete_doc("DocType", source_dt_name, ignore_missing=True, force=True)
			nts.delete_doc("Role", test_role, ignore_missing=True, force=True)
			test_user_doc = nts.get_doc("User", user.name)
			test_user_doc.remove_roles(test_role)
			...
			with self.assertRaises(nts.PermissionError) as cm_link:
				nts.qb.get_query(
					source_dt_name,
	...
	def test_child_table_group_by_order_by_permissions(self):
		...
		nts.set_user("Administrator")
		nts.delete_doc("DocType", child_dt_name, ignore_missing=True, force=True)
		nts.delete_doc("DocType", parent_dt_name, ignore_missing=True, force=True)
		nts.delete_doc("Role", test_role, ignore_missing=True, force=True)

		test_user_doc = nts.get_doc("User", test_user_email)
		test_user_doc.remove_roles(test_role)
	...
	def test_group_by_order_by_validation_errors(self):
		...
		for field in invalid_group_by_fields:
			with self.assertRaises(
				nts.ValidationError, msg=f"Invalid GROUP BY field '{field}' passed validation"
			):
				nts.qb.get_query("User", group_by=field).get_sql()
		...
		for field in invalid_order_by_fields:
			with self.assertRaises(
				(nts.ValidationError, ValueError),
				msg=f"Invalid ORDER BY field '{field}' passed validation",
			):
				nts.qb.get_query("User", order_by=field).get_sql()
	...
	def test_backtick_rejection_group_order(self):
		...
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", group_by="`name`").get_sql()
		self.assertIn("invalid backtick notation", str(cm.exception))

		# Test single backtick with direction (invalid notation)
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", order_by="`name` ASC").get_sql()
		self.assertIn("invalid backtick notation", str(cm.exception))

		# Test multiple single backticks (invalid notation)
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", group_by="`name`, `email`").get_sql()
		self.assertIn("invalid backtick notation", str(cm.exception))

		# Valid backtick notation should work
		nts.qb.get_query("User", group_by="`tabUser`.`name`").get_sql()
		nts.qb.get_query("User", order_by="`tabUser`.`name` ASC").get_sql()
	...
	def test_sql_functions_in_fields(self):
		...
		query = nts.qb.get_query("User", fields=["user_type", {"COUNT": "name"}], group_by="user_type")
		sql = query.get_sql()
		...
		# Test ABS function
		query = nts.qb.get_query("User", fields=[{"ABS": "enabled", "as": "abs_enabled"}])
		sql = query.get_sql()
		...
		query = nts.qb.get_query("User", fields=[{"TIMESTAMP": "creation", "as": "ts"}])
		sql = query.get_sql()
		...
		query = nts.qb.get_query("User", fields=[{"NOW": None, "as": "current_time"}])
		sql = query.get_sql()
		...
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", fields=[{"UNSUPPORTED_FUNC": "name"}]).get_sql()
		...
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", fields=[{"UPPER": ["first_name"]}]).get_sql()
		self.assertIn("Unsupported function or operator: UPPER", str(cm.exception))

		# Test SQL injection attempt
		with self.assertRaises(nts.ValidationError) as cm:
			nts.qb.get_query("User", fields=[{"DROP": "TABLE users"}]).get_sql()
	...
	def test_arithmetic_operators_in_fields(self):
		...
		query = nts.qb.get_query("User", fields=[{"ADD": [1, 2], "as": "sum_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("1+2 `sum_result`"), sql)

		# Test simple subtraction
		query = nts.qb.get_query("User", fields=[{"SUB": [10, 5], "as": "diff_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("10-5 `diff_result`"), sql)

		# Test simple multiplication
		query = nts.qb.get_query("User", fields=[{"MUL": [3, 4], "as": "prod_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("3*4 `prod_result`"), sql)

		# Test simple division
		query = nts.qb.get_query("User", fields=[{"DIV": [10, 2], "as": "div_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("10/2 `div_result`"), sql)

		# Test operator with field names
		query = nts.qb.get_query("User", fields=[{"ADD": ["enabled", "login_after"], "as": "field_sum"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("`enabled`+`login_after` `field_sum`"), sql)

		# Test nested operators
		query = nts.qb.get_query("User", fields=[{"ADD": [{"MUL": [2, 3]}, 4], "as": "nested_result"}])
		sql = query.get_sql()
	...
	def test_share_only_access(self):
		...
		self.addCleanup(lambda: nts.set_user("Administrator"))

		# Verify user can't access without share
		nts.set_user(test_user)
		result = nts.qb.get_query("Event", filters={"name": event.name}, ignore_permissions=False).run()
		self.assertEqual(len(result), 0, "User should not see event without share")

		# Share the document
		nts.set_user("Administrator")
		nts.share.add("Event", event.name, test_user)
	...
	def test_if_owner_constraint_with_shared_docs(self):
		...
		# Share the document
		nts.set_user("Administrator")
		nts.share.add("Test Blog Post", blog_post.name, test_user)
	...
	def test_user_permission_with_shared_docs(self):
		...
		nts.set_user("Administrator")
		nts.share.add("Test Blog Post", blog_post2.name, test_user)
	...
	def test_child_table_filters_orphaned_rows(self):
		...
		self.addCleanup(
			lambda: nts.db.sql("DELETE FROM `tabDefaultValue` WHERE name = '_test_orphan_row'")
		)
		self.addCleanup(lambda: nts.set_user("Administrator"))
	...
	def test_combined_raw_criterion_precedence(self):
		...
		"""
		from nts.database.query import CombinedRawCriterion, RawCriterion
