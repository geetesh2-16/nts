...
class TestRenameDoc(IntegrationTestCase):
	...
	def test_rename_autoincrement_doc(self):
		if not nts.db.exists("DocType", "Autoincrement DocType"):
			new_doctype(
		...
		if not nts.db.exists("DocType", "Autoincrement Linked DocType"):
			new_doctype(
		...
		# create records
		mary = nts.new_doc("Autoincrement DocType", first_name="Mary").insert()
		marilyn = nts.new_doc("Autoincrement DocType", first_name="Marilyn").insert()
		linked_with_marilyn = nts.new_doc(
		...
		nts.rename_doc("Autoincrement DocType", marilyn.name, mary.name, merge=True)
		linked_with_marilyn.reload()

		self.assertTrue(nts.db.exists("Autoincrement DocType", marilyn.name) is None)
		self.assertEqual(linked_with_marilyn.autoincrement_doctype, nts.utils.cstr(mary.name))
