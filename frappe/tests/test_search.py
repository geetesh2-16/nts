...
from nts.permissions import add_user_permission
from nts.tests import IntegrationTestCase
from nts.tests.utils import whitelist_for_tests
...
class TestSearch(IntegrationTestCase):
	...
	def test_search_relevance(self):
		nts.db.set_value("Language", {"name": ("like", "e%")}, "enabled", 1)
	...
	def test_search_link_with_ignore_user_permissions(self):
		...
		for dt in ("Test Search Form", "Test Search Linked"):
			if nts.db.exists("DocType", dt):
				nts.delete_doc("DocType", dt, force=True)
		...
		self.addCleanup(
			lambda: nts.delete_doc("DocType", "Test Search Form", force=True, ignore_missing=True)
		)
		self.addCleanup(lambda: nts.delete_doc("DocType", "Test Search Linked", force=True))

		# Create some test documents
		allowed_doc = nts.get_doc({"doctype": "Test Search Linked", "title": "Allowed Document"}).insert()
		restricted_doc = nts.get_doc(
			{"doctype": "Test Search Linked", "title": "Restricted Document"}
		).insert()
		self.addCleanup(lambda: nts.delete_doc("Test Search Linked", allowed_doc.name, force=True))
		self.addCleanup(lambda: nts.delete_doc("Test Search Linked", restricted_doc.name, force=True))

		# Create a test user with restricted permissions
		test_user = "test_search_user@example.com"
		if not nts.db.exists("User", test_user):
			user = nts.get_doc(
				{
					"doctype": "User",
					"email": test_user,
			...
			self.addCleanup(lambda: nts.delete_doc("User", test_user, force=True))
		...
		self.addCleanup(
			lambda: nts.db.delete("User Permission", {"user": test_user, "allow": "Test Search Linked"})
		)
	...
	def test_search_link_ignore_user_permissions_validation(self):
		...
		self.addCleanup(
			lambda: nts.delete_doc("DocType", "Test Search Linked2", force=True, ignore_missing=True)
		)
		...
		self.addCleanup(lambda: nts.delete_doc("DocType", "Test Search Form Wrong Link", force=True))
...
def use_language(language: str):
	original_lang = nts.local.lang
	nts.local.lang = language

	try:
		yield
	finally:
		nts.local.lang = original_lang
