...
from nts.tests import IntegrationTestCase, MockedRequestTestCase, UnitTestCase
...
class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		...
		original_image_content = open(
			nts.get_app_path("nts", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(nts.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				nts.utils.get_first_day_of_week("2020-12-25"),
				nts.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				nts.utils.get_first_day_of_week("2020-12-20"),
				nts.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			nts.utils.get_first_day_of_week("2020-12-25"),
			nts.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			nts.utils.get_first_day_of_week("2020-12-21"),
			nts.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			nts.utils.get_last_day_of_week("2020-12-24"),
			nts.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			nts.utils.get_last_day_of_week("2020-12-28"),
			nts.utils.getdate("2021-01-02"),
		)
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = nts._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("nts.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": nts.get_doc("UTM Source", "test_source"),
			"utm_medium": nts.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": nts.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
