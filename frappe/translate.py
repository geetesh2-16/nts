...
def get_language(lang_list: list | None = None) -> str:
	...
	return nts.get_system_settings("language") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	...
	return (
		...
		or nts.get_system_settings("language")
		or nts.local.lang
		or "en"
	)
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	if with_language_name:
		...
		languages = nts.client_cache.get_value("languages")
		if not languages:
			languages = nts.get_all("Language", filters={"enabled": 1}, pluck="name")
			nts.client_cache.set_value("languages", languages)
		return languages
