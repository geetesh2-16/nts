...
from nts.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def validate_email_address(email_str, throw=False):
	...
	for name, addr in addresses:
		if not addr:
			if throw:
				nts.throw(
					nts._("{0} is not a valid Email Address").format(
						nts.utils.escape_html(name or email_str)
					),
		...
		if not match:
			if throw:
				nts.throw(
					nts._("{0} is not a valid Email Address").format(nts.utils.escape_html(addr)),
					nts.InvalidEmailAddressError,
				)
			continue
...
def validate_iban(iban: str, throw: bool = False) -> bool:
	...
	if not valid and throw:
		nts.throw(nts._("'{0}' is not a valid IBAN").format(nts.bold(iban)))
...
def has_gravatar(email: str) -> str:
	...
	if nts.flags.in_import or nts.flags.in_install or nts.in_test:
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the nts WSGI."""
...
def get_site_info():
	...
	return orjson.loads(nts.as_json(site_info))
...
def get_db_count(*args):
	...
	return orjson.loads(nts.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	"""
	return orjson.loads(nts.as_json(nts.call(fn, *args, **kwargs)))
...
def get_nts_version() -> str:
	return getattr(nts, "__version__", "unknown")


def get_app_version(app_name: str) -> str:
	try:
		return nts.get_attr(app_name + ".__version__")
	except Exception:
		return "0.0.1"
