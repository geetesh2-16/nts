...
import nts.monitor
from nts import _
from nts.utils import CallbackManager, cint, get_bench_id, get_sites
...
def enqueue(
	...
	**kwargs,
	...
	if not is_async and not nts.in_test:
		from nts.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not nts.in_test)
	if call_directly:
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	try:
		...
		nts.db.rollback(chain=True)
		nts.log_error(title=method_name)
		nts.monitor.add_data_to_monitor(exception=e.__class__.__name__)
...
class ntsWorkerNoFork(ntsWorker):
...
def start_worker_pool(
	...
	if sbool(os.environ.get("nts_BACKGROUND_WORKERS_NOFORK", False)):
		worker_klass = ntsWorkerNoFork
	else:
		import multiprocessing

		multiprocessing.set_start_method("fork", force=True)
		worker_klass = ntsWorker
...
def _check_queue_size(q: Queue):
	max_jobs = cint(nts.conf.max_queued_jobs) or MAX_QUEUED_JOBS
