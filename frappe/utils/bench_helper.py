...
def ntsClickWrapper(cls, handler):
	class Cls(cls):
...
def main():
	...
	ntsClickWrapper(click.Group, handle_exception)(commands=commands)(prog_name="bench")
...
def get_app_groups() -> dict[str, click.Group | click.Command]:
	...
	return dict(
		nts=click.group(
			name="nts", commands=commands, cls=ntsClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=ntsClickWrapper(click.Group, handle_exception)
		)(app_group)
...
def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		...
		return [site_arg]
	elif env_site := os.environ.get("nts_SITE"):
		return [env_site]
