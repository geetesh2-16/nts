...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("nts")},
	}
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	...
	with open(
		os.path.join(dest, hooks.app_name, hooks.app_name, nts.scrub(hooks.app_title), ".nts"), "w"
	) as f:
		f.write("")

	from nts.deprecation_dumpster import boilerplate_modules_txt
...
dynamic = ["version"]
dependencies = [
    # "nts~=16.0.0" # Installed and managed by bench.
]
