...
def get_error_metadata() -> str:
	...
	from nts.utils.logger import sanitized_dict
	...
	try:
		if job := rq.get_current_job():
			...
			metadata["job_name"] = nts.cstr(job.kwargs.get("method"))
			...
			for key in ("method", "path", "referrer"):
				metadata[key] = getattr(nts.local.request, key)
			metadata["form_dict"] = sanitized_dict(nts.form_dict)

		metadata["user"] = getattr(nts.session, "user", "Unidentified")
	except Exception:
		# We don't want to bother with exception handling *while* gathering some error's metadata
		pass
	return nts.as_json(metadata)
