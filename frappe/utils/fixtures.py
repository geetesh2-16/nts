...
def export_fixtures(app=None):
	...
	if app:
		...
		apps = nts.get_installed_apps()
	for app in apps:
		fixture_auto_order = bool(next(iter(nts.get_hooks("fixture_auto_order", app_name=app)), False))
		fixtures = nts.get_hooks("fixtures", app_name=app)
		prefix = None
		for index, fixture in enumerate(fixtures, start=1):
			...
			export_json(
				fixture,
				nts.get_app_path(app, "fixtures", filename + ".json"),
				filters=filters,
