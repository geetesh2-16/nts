...
def get_jenv():
	...
	if jenv := getattr(nts.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	jenv = ntsSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def get_jloader():
	...
	nts.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	if not apps:
		apps = list(reversed(nts.get_installed_apps(_ensure_on_bench=True)))
