...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		if user:
			if user is True:
				user = nts.local.session.get("user")

			key = f"user:{user}:{key}"

		return f"{nts.local.conf.get('db_name')}|{key}".encode()
	...
	def hdel(
		...
		:param shared: shared nts key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in nts.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in nts.local.cache[_name]:
				del nts.local.cache[_name][keys]
			if pipeline:
	...
	def _enable_client_tracking(self, conn):
		try:
			...
			if "client ID" in str(e) and "does not exist" in str(e):
				...
				nts.client_cache.healthy = False
...
class ClientCache:
	...
	This feature is internal to nts Framework and is subjected to change without any notice.
	...
	Usage/Notes:
		...
		- Same keys can be accessed with `nts.cache` too, but that won't implement invalidation.
		...
		- Never use `nts.cache`'s request local cache along with client-side cache. Two
	...
	def get_doc(self, doctype: str, name: str | None = None):
		...
		key = nts.get_document_cache_key(doctype, name)
		return self.get_value(key, generator=lambda: nts.get_doc(doctype, name))
	...
	def erase_persistent_caches(self, *, doctype=None):
		...
		try:
			self.redis.publish(
				...
				json.dumps({"doctype": doctype, "site": nts.local.site}),
			)
		except redis.exceptions.ConnectionError:
	...
	def _handle_persistent_cache_invalidation(self, message):
		...
		payload = nts._dict(json.loads(message["data"]))
