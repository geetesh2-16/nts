...
from nts.locale import get_date_format, get_number_format, get_time_format
...
from nts.utils.inplacevar import protected_inplacevar
from nts.utils.number_format import NumberFormat
...
def is_safe_exec_enabled() -> bool:
	...
	return bool(nts.get_common_site_config(cached=True).get(SAFE_EXEC_CONFIG_KEY))
...
def safe_exec(
	...
	*,
	...
	if not is_safe_exec_enabled():
		...
		msg += f"<br><a href='https://ntsframework.com/docs/user/en/desk/scripting/server-script' target='_blank' rel='noopener noreferrer'>{docs_cta}</a>"
...
def _compile_code(script: str, filename: str, mode: str = "exec"):
	return compile_restricted(script, filename=filename, policy=ntsTransformer, mode=mode)
...
@contextmanager
def safe_exec_flags():
	if nts.flags.in_safe_exec is None:
...
def get_safe_globals():
	...
	out = NamespaceDict(
		...
		nts=NamespaceDict(
			...
			if getattr(nts.local, "session", None) and getattr(nts.local.session, "data", None)
			else "Guest",
			...
			website=NamespaceDict(
				abs_url=nts.website.utils.abs_url,
				extract_title=nts.website.utils.extract_title,
				get_boot_data=nts.website.utils.get_boot_data,
				get_home_page=nts.website.utils.get_home_page,
				get_html_content_based_on_type=nts.website.utils.get_html_content_based_on_type,
			),
		...
		html2text=html2text,
		dev_server=nts._dev_server,
		run_script=run_script,
		is_job_queued=is_job_queued,
		get_visible_columns=get_visible_columns,
	)

	out.nts.update(SAFE_EXCEPTIONS)
...
def get_hooks(hook: str | None = None, default=None, app_name: str | None = None) -> nts._dict:
...
SAFE_DATA_UTILS = {key: nts.utils.data.__dict__[key] for key in VALID_UTILS}
...
SAFE_EXCEPTIONS = get_module_properties(
	nts.exceptions, lambda obj: inspect.isclass(obj) and issubclass(obj, Exception)
)
