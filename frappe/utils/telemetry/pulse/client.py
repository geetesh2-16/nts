...
def is_enabled() -> bool:
	return bool(
		not nts.conf.get("developer_mode", 0)
		and nts.conf.get("pulse_api_key")
		and on_ntscloud()
		...
		eq.add(
			{
				...
				"site": site or nts.local.site,
				"properties": properties,
			},
			interval=interval,
		)
	except Exception as e:
		nts.logger("pulse").error(f"pulse-client - capture failed: {e!s}")
...
def bulk_capture(events):
	...
	for event in events:
		capture(
			...
			site=event.get("site"),
			app=event.get("app"),
			user=event.get("user") or nts.session.user,
...
def post(events):
	...
	url = _get_ingest_url()
	data = nts.as_json({"events": events})
	resp = session.post(url, data=data, timeout=15)
	if not (200 <= resp.status_code < 300):
		msg = f"pulse-client - post failed: {resp.status_code} {resp.text}"
		nts.logger("pulse").error(msg)
		raise Exception(msg)
	return resp


def _create_session():
	api_key = nts.conf.get("pulse_api_key")
...
def _get_ingest_url():
	host = nts.conf.get("pulse_host") or "https://pulse.m.nts.cloud"
	host = ensure_http(host)
	host = host.rstrip("/")

	endpoint = nts.conf.get("pulse_ingest_endpoint") or "/api/method/pulse.api.bulk_ingest"
...
class EventQueue:
	...
	@property
	def length(self):
		return nts.cache.llen(self.queue)
	...
	def _is_ratelimited(self, event, interval):
		...
		last_sent = nts.cache.get_value(last_sent_key)
	...
	def _update_ratelimit(self, event, interval):
		...
		event_key = self._get_event_key(event)
		last_sent_key = f"{self.ratelimit_prefix}{event_key}"
		nts.cache.set_value(last_sent_key, time.monotonic())

	def _queue_event(self, event):
		nts.cache.lpush(self.queue, nts.as_json(event))
		nts.cache.ltrim(self.queue, 0, self.queue_size - 1)
		...
		for _ in range(max_batches):
			...
			try:
				...
				if retry_attempts > max_retries:
					...
					nts.logger("pulse").error(f"pulse-client - max retries reached: {e!s}")
					self._requeue_events(events)
					break

				pending_events = events
				time.sleep(backoff_seconds * (2 ** (retry_attempts - 1)))
				nts.logger("pulse").error(f"pulse-client - retrying batch due to error: {e!s}")

	def collect(self, batch_size=100):
		events = []
		for _ in range(batch_size):
			event_json = nts.cache.rpop(self.queue)
			if not event_json:
				break
	...
	def _requeue_events(self, events):
		...
		for event in reversed(events):
			nts.cache.rpush(self.queue, nts.as_json(event))
		nts.cache.ltrim(self.queue, 0, self.queue_size - 1)

	def _decode_event(self, event_json):
		event_json = event_json.decode()
		with suppress(JSONDecodeError):
			return nts.parse_json(event_json)

	def get_events(self, limit=20):
		events = []
		for _ in range(limit):
			event_json = nts.cache.lindex(self.queue, _)
			if not event_json:
				break
			data = self._decode_event(event_json)
			if data:
	...
	def get_last_sent_events(self, limit=20):
		events = []
		keys = nts.cache.get_keys(f"{self.ratelimit_prefix}*")[:limit]
		for key in keys:
			last_sent = nts.cache.get_value(key)
