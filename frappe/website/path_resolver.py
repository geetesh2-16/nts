...
from nts.website.page_renderers.list_renderer import ListPage
...
from nts.website.utils import can_cache, check_if_webform_exists, get_home_page
...
def resolve_redirect(path, query_string=None):
	...
	def raise_redirect(redirect_location, status_code=301, forward_query_params=False):
		if forward_query_params and query_string:
			...
			redirect_location += separator + nts.safe_decode(query_string)
		nts.flags.redirect_location = redirect_location
		raise nts.Redirect(status_code)

	redirect_to = nts.cache.hget("website_redirects", path or "/")
	if redirect_to:
	...
	redirects += [
		...
		for r in (nts.get_website_settings("route_redirects") or [])
	]
	...
	nts.cache.hset("website_redirects", path_to_match or "/", False)
