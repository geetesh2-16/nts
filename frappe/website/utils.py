...
def delete_page_cache(path=None):
	if path:
		nts.cache.delete_value(f"{WEBSITE_PAGE_CACHE_PREFIX}{path}")
	else:
		nts.cache.delete_keys(WEBSITE_PAGE_CACHE_PREFIX)
...
def can_cache(no_cache=False):
	...
	if getattr(nts.local, "no_cache", False):
		return False
	if nts.request and nts.request.query_string:
		return False
	return not no_cache
...
def get_home_page():
	if nts.local.flags.home_page and not nts.in_test:
	...
	def _get_home_page():
		...
		if home_page == "me" and nts.session.data.user_type == "System User":
			home_page = "desk"
...
def get_boot_data():
	...
	return {
		...
		"time_zone": {
			...
			"user": nts.get_cached_value("User", nts.session.user, "time_zone")
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		...
		no_cache = nts.request and nts.request.cache_control.no_cache
		if can_cache(no_cache):
			html = None
			page_cache = nts.cache.get_value(cache_key)
			if page_cache and nts.local.lang in page_cache:
				html = page_cache[nts.local.lang]
			if html:
				nts.local.response.from_cache = True
				nts.local.response_headers.update(cache_headers)
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = nts.cache.get_value(cache_key) or {}
			page_cache[nts.local.lang] = html
			nts.cache.set_value(cache_key, page_cache, expires_in_sec=30 * 60)
			nts.local.response_headers.update(cache_headers)
...
def check_if_webform_exists(route):
	return nts.db.exists("Web Form", {"name": route.strip("/")}) or nts.db.exists(
