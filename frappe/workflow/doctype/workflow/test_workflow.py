...
class TestWorkflow(IntegrationTestCase):
	...
	def test_dynamic_update_value_expression(self):
		...
		self.workflow.states[1].update_value = "nts.session.user"
		...
		self.assertEqual(todo.assigned_by, nts.session.user)
	...
	def test_static_value_when_expression_disabled(self):
		...
		self.assertEqual(todo.description, "nts.session.user")
	...
	def test_sync_tasks(self, doc=None):
		...
		with self.patch_hooks(
			{
				"workflow_methods": [
					{
						"name": "Create Note",
						"method": "nts.workflow.doctype.workflow.test_workflow.create_new_note",
					}
				]
			}
		):
		...
		self.assertTrue(
			nts.db.exists("Note", {"title": "workflow - " + domain.name, "content": "workflow test"})
		)
		self.assertTrue(nts.db.exists("Domain", {"name": "workflow - " + domain.name}))
		self.assertTrue(nts.db.exists("Webhook Request Log", {"url": "https://workflowtasks.org/post"}))
...
def create_domain_workflow():
	...
	if nts.db.exists("Workflow", "Test Domain"):
		nts.delete_doc("Workflow", "Test Domain")
	...
	pending_to_approved_transition = nts.new_doc("Workflow Transition Tasks")
...
def create_new_todo():
	return nts.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
...
def create_new_server_script():
	server_script = nts.new_doc("Server Script")
