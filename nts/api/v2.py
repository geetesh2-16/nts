...
import nts
import nts.client
from nts import _, cint, cstr, get_newargs, is_whitelisted
...
def document_list(doctype: str) -> list[dict[str, Any]]:
	...
	Query Parameters (accessible via nts.form_dict):
	...
	Response:
		nts.response["data"]: List of document records as dicts
		nts.response["has_next_page"]: Indicates if more pages are available
	...
	Controller Customization:
		...
		Example:
			class Project(Document):
				@staticmethod
				def get_list(query):
					Project = nts.qb.DocType("Project")
					if user_has_role("Project Owner"):
						query = query.where(Project.owner == nts.session.user)
					else:
	...
	args = nts.form_dict
	fields: list | None = nts.parse_json(args.get("fields", None))
	filters: dict | None = nts.parse_json(args.get("filters", None))
	...
	if hasattr(controller, "get_list"):
		try:
			...
			nts.throw(_("Error in {0}.get_list: {1}").format(doctype, str(e)))

	data = query.run(as_dict=as_dict, debug=debug)
	nts.response["has_next_page"] = len(data) > limit
	return data[:limit]
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	result = doc.run_method(method, **nts.form_dict)
	nts.response.docs.append(doc.as_dict())
	return result
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	doc = nts.get_doc(document, check_permission=PERMISSION_MAP[nts.request.method])
