...
class HTTPRequest:
	...
	def set_request_ip(self):
		...
		elif nts.request and getattr(nts.request, "remote_addr", None):
			nts.local.request_ip = nts.request.remote_addr
	...
	def is_allowed_referrer(self):
		referrer = nts.get_request_header("Referer")
		origin = nts.get_request_header("Origin")

		# Get the list of allowed referrers from cache or configuration
		allowed_referrers = nts.cache.get_value(
			"allowed_referrers",
			generator=lambda: nts.conf.get("allowed_referrers", []),
		)
...
class LoginManager:
	...
	def __init__(self):
		...
		if nts.local.form_dict.get("cmd") == "login" or nts.local.request.path == "/api/method/login":
			...
			except (AttributeError, nts.DoesNotExistError):
				self.user = "Guest"
				self.get_user_info()
				self.make_session()
				self.set_user_info()
	...
	def setup_boot_cache(self):
		...
		nts.cache_manager.build_domain_restricted_doctype_cache()
		nts.cache_manager.build_domain_restricted_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			nts.local.cookie_manager.set_cookie("system_user", "no", deduplicate=True)
			if not resume:
				nts.local.response["message"] = "No App"
				nts.local.response["home_page"] = get_default_path() or "/" + get_home_page()
		else:
			nts.local.cookie_manager.set_cookie("system_user", "yes", deduplicate=True)
			if not resume:
				nts.local.response["message"] = "Logged In"
				nts.local.response["home_page"] = get_default_path() or "/desk"
		...
		nts.local.cookie_manager.set_cookie("full_name", self.full_name, deduplicate=True)
		nts.local.cookie_manager.set_cookie("user_id", self.user, deduplicate=True)
		nts.local.cookie_manager.set_cookie("user_image", self.info.user_image or "", deduplicate=True)
		nts.local.cookie_manager.set_cookie("user_lang", nts.local.lang, deduplicate=True)
	...
	def force_user_to_reset_password(self):
		...
		reset_pwd_after_days = cint(nts.get_system_settings("force_user_to_reset_password"))
	...
	def validate_hour(self):
		...
		login_before = cint(nts.db.get_value("User", self.user, "login_before", ignore=True))
		login_after = cint(nts.db.get_value("User", self.user, "login_after", ignore=True))
...
class CookieManager:
	...
	def set_cookie(
		...
		if (
			...
			and (request := getattr(nts.local, "request", None))
...
def clear_cookies():
	if hasattr(nts.local, "session"):
		nts.session.sid = ""
	nts.local.cookie_manager.delete_cookie(
...
def validate_ip_address(user):
	...
	user_info = nts.get_cached_doc("User", user)
	...
	bypass_restrict_ip_check = nts.get_system_settings("bypass_restrict_ip_check_if_2fa_enabled")

	# check if two factor auth is enabled
	if nts.get_system_settings("enable_two_factor_auth") and not bypass_restrict_ip_check:
...
def check_request_ip():
	if nts.local.request_ip is None:
		nts.local.request_ip = "127.0.0.1"
