...
def build_missing_files():
	...
	development = nts.local.conf.developer_mode or nts._dev_server
...
def get_assets_link(nts_head) -> str:
	...
	tag = getoutput(
		r"cd ../apps/nts && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(nts_head)
	)
...
def download_nts_assets(verbose=True) -> bool:
	"""Download and set up nts assets if they exist based on the current commit HEAD.
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `nts.templates`."""
