...
def build(
	...
	with filelock("bench_build", is_global=True, timeout=10):
		...
		esbuild_target = nts.local.conf.get("esbuild_target") or os.environ.get("ESBUILD_TARGET")
...
def execute(context: CliCtxObj, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = nts.get_attr(method)(*fn_args, **fn_kwargs)
			except Exception:
...
nts.connect()
nts.local.lang = nts.get_system_settings('language')
nts.db.connect()
```
	"""
	)
...
def setup_chrome():
	from nts.utils.print_utils import setup_chromium
