...
import nts
from nts import _dict, get_file_json
from nts.exceptions import IncorrectSitePath
...
def get_site_config(
	...
	*,
	...
	sites_path = sites_path or getattr(nts.local, "sites_path", ".")
	site_path = site_path or getattr(nts.local, "site_path", None)
...
def _get_site_config(sites_path: str, site_path: str) -> _dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			try:
				...
				click.secho(f"{nts.local.site}/site_config.json is invalid", fg="red")
				print(error)
				raise
		elif nts.local.site and not nts.local.flags.new_site:
			error_msg = f"{nts.local.site} does not exist."
	...
	if config["db_type"] in ("mariadb", "postgres"):
		...
		config["db_user"] = os.environ.get("nts_DB_USER") or config.get("db_user") or config.get("db_name")

		# read password
		config["db_password"] = os.environ.get("nts_DB_PASSWORD") or config.get("db_password")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("nts_DB_NAME") or config.get("db_name") or config["db_user"]
