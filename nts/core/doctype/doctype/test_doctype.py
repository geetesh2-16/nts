...
class TestDocType(IntegrationTestCase):
	...
	def test_meta_serialization(self):
		...
		doc = nts.new_doc(doctype.name, some_fieldname="something").insert()
		doc.save()
		doc.submit()
		nts.get_meta(doctype.name).as_dict()
	...
	def test_decimal_field_configuration(self):
		...
		decimal_field_type = nts.db.get_column_type(doctype.name, "decimal_field")
	...
	def test_decimal_field_precision_exceeds_length(self):
		...
		self.assertRaises(nts.ValidationError, doctype.insert)

	def test_delete_doc_clears_cache(self):
		dt = new_doctype(
			fields=[{"fieldname": "test_fdname", "fieldtype": "Data", "label": "Test Field"}],
		).insert()
		nts.get_meta(dt.name)
		nts.delete_doc("DocType", dt.name, force=1, delete_permanently=False)
		nts.db.commit()
		with self.assertRaises(nts.DoesNotExistError):
			nts.get_meta(dt.name)
