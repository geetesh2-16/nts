...
class TestDocumentNamingRule(IntegrationTestCase):
	def test_naming_rule_by_series(self):
		...
		todo = nts.get_doc(
			doctype="ToDo", description="Is this my name " + nts.generate_hash()
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		todo = nts.get_doc(
			doctype="ToDo", priority="High", description="Is this my name " + nts.generate_hash()
		).insert()

		todo_1 = nts.get_doc(
			doctype="ToDo", priority="Medium", description="Is this my name " + nts.generate_hash()
		).insert()

		todo_2 = nts.get_doc(
			doctype="ToDo", priority="Low", description="Is this my name " + nts.generate_hash()
		).insert()
