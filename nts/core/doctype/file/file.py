...
from nts.utils.html_utils import escape_html
from nts.utils.image import optimize_image, strip_exif_data
from nts.utils.pdf import pdf_contains_js
...
class File(Document):
	...
	def enforce_public_file_restrictions(self):
		if not self.is_private and nts.get_system_settings(
			"only_allow_system_managers_to_upload_public_files"
		):
			try:
				nts.only_for("System Manager")
			except PermissionError:
				nts.throw(_("Only System Managers can make this file public."))
	...
	def check_content(self):
		if self.file_type == "PDF" and self._content and pdf_contains_js(self._content):
			nts.throw(_("This PDF cannot be uploaded as it contains unsafe content."))
