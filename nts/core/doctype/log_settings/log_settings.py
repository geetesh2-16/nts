...
def clear_log_table(doctype, days=90):
	...
	try:
		if nts.db.db_type == "postgres":
			nts.db.sql_ddl(f'CREATE TABLE "{temporary}" (LIKE "{original}" INCLUDING ALL)')
			...
			nts.db.sql_ddl(f'ALTER TABLE "{original}" RENAME TO "{backup}"')
			nts.db.sql_ddl(f'ALTER TABLE "{temporary}" RENAME TO "{original}"')
