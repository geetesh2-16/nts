...
class TestRQJob(IntegrationTestCase):
	...
	def setUp(self) -> None:
		...
		for job in nts.get_all("RQ Job", {"status": "queued"}):
			nts.get_doc("RQ Job", job.name).cancel()
		return super().setUp()
	...
	def test_get_list_filtering(self):
		...
		remove_failed_jobs()
		jobs = nts.get_all("RQ Job", {"status": "failed"})
		...
		non_failed_jobs = nts.get_all("RQ Job", {"status": ("!=", "failed")})
