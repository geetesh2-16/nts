...
class ScheduledJobType(Document):
	...
	def execute(self):
		if nts.job:
			nts.job.frequency = self.frequency
			nts.job.cron_format = self.cron_format
...
def skip_next_execution(doc: str):
	nts.only_for("System Manager")
	doc = json.loads(doc)
	doc: ScheduledJobType = nts.get_doc("Scheduled Job Type", doc.get("name"))
...
def run_scheduled_job(scheduled_job_type: str, job_type: str | None = None):
	...
	try:
		nts.get_doc("Scheduled Job Type", scheduled_job_type).execute()
	except Exception:
