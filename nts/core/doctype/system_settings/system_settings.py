...
def get_system_settings(key: str):
	...
	if not (system_settings := getattr(nts.local, "system_settings", None)):
		try:
			system_settings = nts.client_cache.get_doc("System Settings")
			nts.local.system_settings = system_settings
		except nts.DoesNotExistError:  # possible during new install
...
def clear_system_settings_cache():
	nts.client_cache.delete_value(nts.get_document_cache_key("System Settings", "System Settings"))
