...
class TestUser(IntegrationTestCase):
	...
	def test_delete(self):
		...
		user = nts.copy_doc(self.globalTestRecords["User"][1])
		...
		nts.copy_doc(self.globalTestRecords["Role"][1]).insert()
	...
	def test_password_strength(self):
		...
		with change_settings("System Settings", enable_password_policy=1, minimum_password_score=1):
			...
			self.assertRaises(
				nts.exceptions.ValidationError, handle_password_test_fail, result["feedback"]
			)
			self.assertRaises(
	...
	def test_signup(self):
		...
		self.assertEqual(
			nts.cache.hget("redirect_after_login", random_user), sanitize_redirect("/welcome")
		)
