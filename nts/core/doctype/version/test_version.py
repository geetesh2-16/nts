...
import nts
from nts.core.doctype.version.version import (
	_as_string,
	_generate_html_diff,
...
class TestVersion(IntegrationTestCase):
	def test_onload_generates_html_diffs_for_multiline(self):
		...
		version = nts.get_doc(
			...
			ref_doctype="ToDo",
			docname="test-doc",
			data=nts.as_json({"changed": [["description", "line1\nline2", "line1\nmodified"]]}),
		)
	...
	def test_onload_generates_html_diffs_for_long_text(self):
		...
		version = nts.get_doc(
			...
			ref_doctype="ToDo",
			docname="test-doc",
			data=nts.as_json({"changed": [["notes", "x" * 81, "y" * 81]]}),
		)
	...
	def test_onload_no_html_diffs_for_simple_changes(self):
		...
		version = nts.get_doc(
			...
			ref_doctype="ToDo",
			docname="test-doc",
			data=nts.as_json({"changed": [["status", "Open", "Closed"]]}),
		)
	...
	def test_onload_handles_empty_data(self):
		...
		version.data = nts.as_json({"changed": []})
		version.onload()
		self.assertIsNone(version.get_onload().get("html_diffs"))
