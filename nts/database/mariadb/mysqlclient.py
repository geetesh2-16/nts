...
from nts.utils import get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if nts.conf.db_ssl_ca:
			ssl_config = {
				"ca": nts.conf.db_ssl_ca,
				"check_hostname": nts.conf.db_ssl_check_hostname,
			}

			# Add client certificates for mutual SSL if available
			if nts.conf.db_ssl_cert and nts.conf.db_ssl_key:
				ssl_config.update({"cert": nts.conf.db_ssl_cert, "key": nts.conf.db_ssl_key})
...
def escape_frozenset(obj, mapping=None):
	return nts.local.db._conn.literal(tuple(obj))
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		s = nts.as_unicode(escape_string(s)).replace("`", "\\`") if s else ""
	...
	def get_table_columns_description(self, table_name):
		...
		return self.sql(
			...
			where table_name = '{table_name}'
   			and table_schema = '{nts.db.cur_db_name}' """,
			as_dict=1,
		)
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = nts.client_cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			nts.client_cache.set_value("db_tables", tables)
