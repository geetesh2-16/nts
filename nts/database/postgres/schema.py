...
class PostgresTable(DBTable):
	...
	def alter(self):
		...
		try:
			...
			if change_nullability:
				# nosemgrep
				nts.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and nts.db.get_column_type(self.doctype, "name") != "uuid":
		...
		if autoname != "UUID" and nts.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({nts.db.VARCHAR_LEN})"
