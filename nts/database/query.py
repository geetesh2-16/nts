...
from nts.database.operator_map import NESTED_SET_OPERATORS, OPERATOR_MAP
...
from nts.model import OPTIONAL_FIELDS, get_permitted_fields
...
from nts.query_builder.custom import Month, MonthName, Quarter
...
def _apply_datetime_field_filter_conversion(between_values: tuple | list, doctype: str, field) -> tuple:
	...
	"""
	from nts.model.db_query import _convert_type_for_between_filters
...
class Engine:
		...
		*,
		...
		if limit:
			if not isinstance(limit, int) or limit < 0:
				nts.throw(_("Limit must be a non-negative integer"), TypeError)
			self.query = self.query.limit(limit)

		if offset:
			if not isinstance(offset, int) or offset < 0:
				nts.throw(_("Offset must be a non-negative integer"), TypeError)
		...
		if isinstance(filters, list | tuple):
			...
			if is_nested_structure:
				...
				try:
					...
					nts.throw(_("Error parsing nested filters: {0}. {1}").format(filters, e), exc=e)
	...
	def _build_criterion_for_simple_filter(
		...
		if operator.lower() in additional_filters_config:
			f = nts._dict(doctype=doctype or self.doctype, fieldname=field, operator=operator, value=value)
			from nts.model.db_query import get_additional_filter_field
		...
		if isinstance(value, Document):
			nts.throw(_("Document cannot be used as a filter value"))
		_operator = operator

		if _operator.lower() in ("timespan", "previous", "next"):
			from nts.model.db_query import get_date_range
		...
		if _operator.lower() == "between":
			if isinstance(_value, list | tuple) and len(_value) == 2:
				...
				from nts.model.db_query import get_between_date_filter

				target_meta = nts.get_meta(doctype or self.doctype)
		...
		if _operator in NESTED_SET_OPERATORS:
			...
			main_meta = nts.get_meta(self.doctype)
	...
	def _parse_nested_filters(self, nested_list: list | tuple) -> "Criterion | None":
		...
		if not isinstance(nested_list, list | tuple):
			nts.throw(_("Nested filters must be provided as a list or tuple."))
		...
		while idx < len(nested_list):
			...
			if idx >= len(nested_list):
				nts.throw(_("Filter condition missing after operator: {0}").format(operator_str))
	...
	def _condition_to_criterion(self, condition: list | tuple) -> "Criterion":
		...
		if not isinstance(condition, list | tuple):
			nts.throw(_("Invalid condition type in nested filters: {0}").format(type(condition)))
		...
		if is_nested:
			...
			if (
				...
				nts.throw(_("Invalid simple filter format: {0}").format(condition))
	...
	def _validate_and_prepare_filter_field(self, field: str | Field, doctype: str | None = None) -> Field:
		...
		if "`" in field:
			if parsed := self._parse_backtick_field_notation(field):
				...
				return nts.qb.DocType(table_name)[field_name]
		...
		if "." in field:
			...
			if doctype and doctype != self.doctype:
				...
				parent_meta = nts.get_meta(self.doctype)
				...
				if (
					...
					for df in meta.get_table_fields(include_computed=True):
						try:
							child_meta = nts.get_meta(df.options)
						except nts.DoesNotExistError:
							continue
				...
				return nts.qb.DocType(target_doctype)[target_fieldname]
	...
	def parse_string_field(self, field: str):
		...
		if not match:
			nts.throw(_("Could not parse field: {0}").format(field))
		...
		if table_name:
			...
			if not TABLE_NAME_PATTERN.match(table_name.lstrip("tab")):
				nts.throw(_("Invalid characters in table name: {0}").format(table_name))

			doctype_name = table_name[3:] if table_name.startswith("tab") else table_name
			table_obj = nts.qb.DocType(doctype_name)
	...
	def parse_fields(
		...
		if isinstance(fields, str):
			...
			nts.throw(_("Fields must be a string, list, tuple, pypika Field, or pypika Function"))
	...
	def _parse_single_field_item(
		...
		if not isinstance(field, str):
			nts.throw(_("Invalid field type: {0}").format(type(field)))
	...
	def _validate_group_by(self, group_by: str) -> list[Field]:
		...
		if not isinstance(group_by, str):
			nts.throw(_("Group By must be a string"), TypeError)
	...
	def _validate_order_by(self, order_by: str) -> list[tuple[Field | str, Order]]:
		...
		if not isinstance(order_by, str):
			nts.throw(_("Order By must be a string"), TypeError)
	...
	def _raise_permission_error(self, doctype=None):
		nts.throw(
			_("Insufficient Permission for {0}").format(nts.bold(doctype or self.doctype)),
			nts.PermissionError,
		)
		...
		for field in self.fields:
			if isinstance(field, ChildTableField):
				...
				if field.link_fieldname in permitted_fields_set:
					...
					has_target_perm = nts.has_permission(
						target_doctype, "select", user=self.user
					) or nts.has_permission(target_doctype, "read", user=self.user)
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				...
				if docs:
					...
					strict_user_permissions = nts.get_system_settings("apply_strict_user_permissions")
	...
	def add_permission_conditions(self):
		...
		if self.permission_doctype != self.doctype:
			parent_meta = nts.get_meta(self.permission_doctype)
	...
	def get_permission_conditions(self, doctype: str, table: Table) -> Criterion | None:
		role_permissions = nts.permissions.get_role_permissions(doctype, user=self.user)
		...
		if not has_role_permission:
			...
			shared_docs = nts.share.get_shared(doctype, self.user)
			if not shared_docs:
	...
	def get_permission_query_conditions(self, doctype: str | None = None) -> list["RawCriterion"]:
		...
		for method in condition_methods:
			if c := nts.call(nts.get_attr(method), self.user, doctype=doctype):
	...
	def get_permission_type(
		...
		if nts.only_has_select_perm(doctype, user=self.user):
			return "select"
		return "read"
	...
	def _is_field_nullable(self, doctype: str, fieldname: str) -> bool:
		...
		try:
			...
			if (meta := nts.client_cache.get_value(f"doctype_meta::{doctype}")) is None:
				return True
	...
	def _get_ifnull_fallback(self, doctype: str, fieldname: str) -> str:
		...
		try:
			...
			if nts.db.db_type == "postgres":
				...
				target_table = nts.utils.get_table_name(doctype)
				info_schema = nts.qb.Schema("information_schema")
				columns = info_schema.columns
				current_schema = nts.conf.get("db_schema", "public")
				res = (
					nts.qb.from_(columns)
		...
		try:
			db_type_info = nts.db.type_map.get(fieldtype, ("varchar",))
			if db_type_info:
	...
	def parse(field: str, doctype: str, allow_tab_notation: bool = True):
		if "." in field:
			...
			if child_match:
				...
				try:
					meta = nts.get_meta(doctype)  # Get meta of the *parent* doctype
...
class RawCriterion(Term):
	...
	Allows using raw SQL strings in pypika queries:
		nts.qb.from_("DocType").where(RawCriterion("name like 'a%'"))
	"""
...
class SQLFunctionParser:
	...
	def _extract_dict_components(self, d: dict, valid_keys: dict, error_msg: str) -> tuple:
		...
		if not name:
			nts.throw(_("Invalid {0} dictionary format").format(error_msg), nts.ValidationError)

		if name not in valid_keys:
			nts.throw(_("Unsupported {0}: {1}").format(error_msg, name), nts.ValidationError)
	...
	def _validate_string_argument(self, arg: str, *, function_name: str | None = None):
		...
		if not arg:
			nts.throw(_("Empty string arguments are not allowed"), nts.ValidationError)
	...
	def _validate_alias(self, alias: str):
		...
		if not isinstance(alias, str):
			nts.throw(_("Alias must be a string"), nts.ValidationError)

		alias = alias.strip()
		if not alias:
			nts.throw(_("Empty alias is not allowed"), nts.ValidationError)
