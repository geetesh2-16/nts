...
class DBTable:
	...
	def sync(self):
		...
		if self.is_new():
			...
			nts.client_cache.delete_value(f"table_columns::{self.table_name}")
			self.alter()
...
class DbColumn:
	...
	def build_for_alter_table(self, current_def):
		...
		if current_def["type"] != column_type and not (
			...
			current_def["type"] == "longtext" and column_type == "json" and nts.db.db_type == "mariadb"
		):
	...
	def default_changed(self, current_def):
		if "decimal" in current_def["type"]:
			...
			db_field_type = nts.db.type_map.get(fieldtype)
...
def get_definition(fieldtype, precision=None, length=None, *, options=None):
	...
	if (
		...
		and nts.db.exists("DocType", options)
		and nts.get_meta(options).autoname == "UUID"
	):
		d = ("uuid", None)
...
def add_column(doctype, column_name, fieldtype, precision=None, length=None, default=None, not_null=False):
	...
	if nts.db.db_type == "sqlite":
		if column_name in nts.db.get_table_columns(doctype):
			return
