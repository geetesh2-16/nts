...
class SQLiteTable(DBTable):
	def create(self):
		...
		nts.db.sql_ddl(create_table_query)

		# Create indexes separately
		for index_query in index_defs:
			nts.db.sql_ddl(index_query)
	...
	def alter(self):
		...
		existing_columns = []
		for column in nts.db.sql(f"PRAGMA table_info(`{self.table_name}`)", as_dict=1):
		...
		nts.db.sql_ddl(create_table)

		# Copy data
		existing_columns = [col.split()[0] for col in existing_columns]
		column_list = ", ".join(existing_columns)
		nts.db.sql_ddl(f"INSERT INTO `{temp_table}` SELECT {column_list} FROM `{self.table_name}`")

		# Drop old table
		nts.db.sql_ddl(f"DROP TABLE `{self.table_name}`")

		# Rename new table
		nts.db.sql_ddl(f"ALTER TABLE `{temp_table}` RENAME TO `{self.table_name}`")
		...
		if self.meta.sort_field == "modified" and not nts.db.get_column_index(
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and nts.db.get_column_type(self.doctype, "name") != "TEXT":
			if not nts.db.get_value(self.doctype, {}, order_by=None):
