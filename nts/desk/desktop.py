...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.restricted_doctypes = (
			nts.cache.get_value("domain_restricted_doctypes") or build_domain_restricted_doctype_cache()
		)
		self.restricted_pages = (
			nts.cache.get_value("domain_restricted_pages") or build_domain_restricted_page_cache()
		)
	...
	for page in all_pages:
		try:
			...
			if not page["app"] and page["module"]:
				page["app"] = nts.db.get_value("Module Def", page["module"], "app_name") or get_module_app(
					page["module"]
				)
