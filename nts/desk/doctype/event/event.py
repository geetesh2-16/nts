...
from nts.model.utils.user_settings import get_user_settings, sync_user_settings, update_user_settings
...
class Event(Document):
	...
	if TYPE_CHECKING:
		...
		from nts.desk.doctype.event_notifications.event_notifications import EventNotifications
...
def update_attending_status(event_name, attendee, status):
	event_doc = nts.get_doc("Event", event_name)

	if event_doc.owner == attendee == nts.session.user:
		nts.db.set_value("Event", event_name, "attending", status)
		return

	for participant in event_doc.event_participants:
		if participant.email == attendee:
			nts.db.set_value("Event Participants", participant.name, "attending", status)
			return

	if not has_permission(event_doc, user=attendee):
		nts.throw(_("You are not allowed to update the status of this event."))
...
def get_permission_query_conditions(user):
	...
	query = f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={nts.db.escape(user)})"""
	if shared_events := nts.share.get_shared("Event", user=user):
		query += f" or `tabEvent`.`name` in ({', '.join([nts.db.escape(e) for e in shared_events])})"

	query += f" or exists (select 'x' from `tabEvent Participants` ep where ep.parent=`tabEvent`.name and ep.email={nts.db.escape(user)})"
...
def get_events(
	...
	type EventLikeDict = Event | nts._dict
