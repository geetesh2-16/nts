...
from nts.modules.export_file import strip_default_fields
from nts.modules.utils import create_directory_on_app_path
...
class WorkspaceSidebar(Document):
	...
	if TYPE_CHECKING:
		from nts.desk.doctype.workspace_sidebar_item.workspace_sidebar_item import WorkspaceSidebarItem
	...
	def __init__(self, *args, **kwargs):
		...
		self.restricted_doctypes = nts.cache.get_value("domain_restricted_doctypes")
		self.restricted_pages = nts.cache.get_value("domain_restricted_pages")
	...
	def export_sidebar(self):
		allow_export = self.app and not nts.flags.in_import and nts.conf.developer_mode
		if allow_export:
			folder_path = create_directory_on_app_path("workspace_sidebar", self.app)
			file_path = os.path.join(folder_path, f"{nts.scrub(self.title)}.json")
	...
	def get_module_from_items(self):
		...
		for item in self.items:
			if item.type != "Section Break" and item.type != "Sidebar Item Group" and item.link_type != "URL":
				try:
					all_modules_in_sidebars.append(nts.get_doc(item.link_type, item.link_to).module)
				except nts.DoesNotExistError as e:
					nts.logger().error(e)
...
def delete_file(app, title):
	...
	file_path = os.path.join(folder_path, f"{nts.scrub(title)}.json")
...
def create_workspace_sidebar_for_workspaces():
	...
	existing_sidebars = nts.get_all("Workspace Sidebar", pluck="title")
	for workspace in all_workspaces:
		if workspace not in existing_sidebars:
			workspace_doc = nts.get_doc("Workspace", workspace)
			sidebar = nts.new_doc("Workspace Sidebar")
			sidebar.title = workspace
			sidebar.header_icon = nts.db.get_value("Workspace", workspace, "icon")
			...
			workspace_sidebar_item = nts.new_doc("Workspace Sidebar Item")
			...
			try:
				...
				nts.log_error(title="Failed To Create Sidebar", message=e)


@nts.whitelist()
def add_sidebar_items(sidebar_title, sidebar_items):
	sidebar_items = loads(sidebar_items)
	title = f"{sidebar_title}-{nts.session.user}"
	w = nts.get_doc("Workspace Sidebar", sidebar_title)
	if not nts.conf.developer_mode:
		try:
			w = nts.get_doc("Workspace Sidebar", title)
		except nts.DoesNotExistError:
			nts.clear_messages()
			w = nts.copy_doc(w, ignore_no_copy=False)
			w.title = title
			w.for_user = nts.session.user
	items = []
	current_idx = 1
	for item in sidebar_items:
		si = nts.new_doc("Workspace Sidebar Item")
		si.update(item)
		si.idx = current_idx
		items.append(si)
		current_idx += 1
		...
		if nested_items:
			for nested_item in nested_items:
				new_nested_item = nts.new_doc("Workspace Sidebar Item")
...
def add_to_my_workspace(workspace):
	try:
		...
		existing_sidebar = nts.db.exists("Workspace Sidebar", sidebar_name)

		if existing_sidebar:
			private_sidebar = nts.get_doc("Workspace Sidebar", existing_sidebar)
		else:
			# clone sidebar
			base_sidebar = nts.get_doc("Workspace Sidebar", "My Workspaces")
			private_sidebar = nts.copy_doc(base_sidebar)
		...
		nts.log_error(title="Error in Adding Private Workspaces", message=e)
...
def auto_generate_sidebar_from_module():
	...
	sidebars = []
	for module in nts.get_all("Module Def", pluck="name"):
		if not (
			nts.db.exists("Workspace Sidebar", {"module": module})
			or nts.db.exists("Workspace Sidebar", {"name": module})
		):
			...
			sidebar.app = nts.local.module_app.get(nts.scrub(module), None)
...
def get_module_info(module_name):
	...
	for entity in entities:
		...
		if entity.lower() == "page":
			...
			pluck = None
		module_info[entity] = nts.get_all(
	...
	if len(doctype_names) > doctype_limit:
		try:
			...
			for doctype in doctype_names:
				if not is_single_doctype(doctype) and not nts.get_meta(doctype).is_virtual:
					doctype_count_map[doctype] = nts.db.count(doctype)
			top_doctypes = [
				name
...
def create_sidebar_items(module_info):
	...
	for entity, items in module_info.items():
		...
		for item in items:
			...
			sidebar_item = nts.new_doc("Workspace Sidebar Item")
...
def add_section_breaks(label, idx):
	section_break = nts.new_doc("Workspace Sidebar Item")
