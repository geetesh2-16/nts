...
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return (follow_document(doctype, doc_name, nts.session.user) and True) or False
	else:
...
def follow_document(doctype, doc_name, user):
	...
	if not nts.get_meta(doctype).track_changes:
		nts.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		nts.toast(_("Administrator can't follow"))
		return False

	if not nts.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		nts.toast(_("Document follow is not enabled for this user."))
		return False
	...
	if not is_document_followed(doctype, doc_name, user):
		...
		doc.save()
		nts.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		nts.delete_doc("Document Follow", doc[0].name, force=True)
		nts.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
