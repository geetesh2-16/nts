...
from nts.email.ntsmail import ntsMail
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		...
		service: DF.Literal[
			"", "nts Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if nts.local.flags.in_patch or nts.in_test:
			return
		...
		if (
			not nts.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "nts Mail"
		):
	...
	@nts.whitelist()
	def validate_nts_mail_settings(self):
		if self.service == "nts Mail":
			nts_mail_client = self.get_nts_mail_client()
			nts_mail_client.validate()
	...
	def clear_cache(self):
		super().clear_cache()
		nts.client_cache.delete_value("automatic_linking_email")
	...
	@property
	def _password(self):
		raise_exception = not (self.auth_method == "OAuth" or self.no_smtp_authentication or nts.in_test)
	...
	def get_nts_mail_client(self):
		return self._nts_mail_client

	@functools.cached_property
	def _nts_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return ntsMail(self.nts_mail_site, self.email_id, access_token=access_token)

			nts.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					nts.bold(self.email_account_name)
				),
				title=_("nts Mail OAuth Error"),
			)
		else:
			return ntsMail(
				self.nts_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "nts Mail":
				nts_mail_client = self.get_nts_mail_client()
				messages = nts_mail_client.pull_raw(last_received_at=self.last_received_at)
...
def pull_emails(email_account: str) -> None:
	...
	nts.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		nts.msgprint(_("Emails are already being pulled from this account."))
...
def get_automatic_email_link():
	...
	return nts.client_cache.get_value("automatic_linking_email", generator=check_db)
