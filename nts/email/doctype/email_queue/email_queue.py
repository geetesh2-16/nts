...
import nts
from nts import _, are_emails_muted, safe_encode, task
...
class EmailQueue(Document):
	...
	def send(
		...
		with SendMailContext(self, smtp_server_instance, nts_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not nts.in_test or nts.flags.testing_email:
					if ctx.email_account_doc.service == "nts Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.nts_mail_client.send_raw(
							sender=self.sender,
			...
			if nts.in_test and not nts.flags.testing_email:
	...
	def clear_old_logs(days=30):
		...
		# Delete queue table
		(
			nts.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
...
from nts.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		self.nts_mail_client: ntsMail = nts_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "nts Mail":
			if not self.nts_mail_client:
				self.nts_mail_client = self.email_account_doc.get_nts_mail_client()
...
def retry_sending(queues: str | list[str]):
	if not nts.has_permission("Email Queue", throw=True):
		return
...
class QueueBuilder:
	...
	def prepare_email_content(self):
		...
		if self.in_reply_to:
			if message_id := nts.db.get_value("Communication", self.in_reply_to, "message_id"):
	...
	def send_emails(self, queue_data, final_recipients):
		...
		nts_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.final_bcc()]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not nts_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "nts Mail":
					nts_mail_client = email_account.get_nts_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, nts_mail_client=nts_mail_client)
