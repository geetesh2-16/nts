...
from nts.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition and not self.filters:
			return _("Yes")
		try:
			doc = nts.get_cached_doc(self.document_type, preview_document)
			if self.condition_type == "Python":
				context = get_context(doc)
				return _("Yes") if nts.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			if self.channel == "SMS":
				return nts.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return nts.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				nts.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				nts.throw(_("Please specify the minutes offset"))
		...
		if self.attach_files == "From Field" and not self.from_attach_field:
			nts.throw(_("Please specify the field from which to attach files"))
...
"""
	...
	def validate_filters(self):
		...
		dummy_doc = nts.new_doc(self.document_type)
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = nts.get_lazy_doc(self.document_type, d.name)

			if (
				self.condition_type == "Python"
				and self.condition
				and not nts.safe_eval(self.condition, None, get_context(doc))
			):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		return enqueue(
			"nts.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
			now=nts.in_test,
	...
	def send_an_email(self, doc, context):
		...
		if doc.doctype != "Communication":
			...
			comm = nts.get_lazy_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=nts.utils.strip_html_tags(nts.render_template(self.message, context)),
		)
	...
	def get_print(self, doc):
		...
		if (doc.docstatus == 0 and not print_settings.allow_print_for_draft) or (
			...
			return {
				...
				or nts.db.get_value("Print Format", self.print_format, "default_print_language")
...
def clear_notification_cache():
	nts.client_cache.delete_keys("notifications::")
...
def get_context(doc):
	nts = namedtuple("nts", ["nts"])
	nts = nts(nts=get_safe_globals().get("nts"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"nts": nts.nts,
