...
import nts
from nts.integrations.doctype.oauth_client.oauth_client import OAuthClient
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		if nts.flags.integration_request_doc:
			nts.flags.integration_request_doc.log_error()
		else:
			nts.log_error()
		raise exc
...
def validate_dynamic_client_metadata(client: OAuth2DynamicClientMetadata):
	...
	if not nts.conf.developer_mode and any(c.scheme != "https" for c in client.redirect_uris):
...
def create_new_oauth_client(client: OAuth2DynamicClientMetadata):
	doc = cast(OAuthClient, nts.get_doc({"doctype": "OAuth Client"}))
...
def get_oauth_settings():
	...
	settings: dict[str, Any] = nts._dict({"skip_authorization": None})
	if nts.get_cached_value("OAuth Settings", "OAuth Settings", "skip_authorization"):
		settings["skip_authorization"] = "Auto"  # based on legacy OAuth Provider Settings value

	elif value := nts.get_cached_value(
