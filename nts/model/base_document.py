...
from nts.utils.defaults import get_not_null_defaults
...
def get_controller(doctype):
	...
	For `custom` type, return `nts.model.document.Document`.
...
def import_controller(doctype):
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = nts.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = nts.bold(f"{class_.__module__}.{class_.__name__}")
			nts.throw(
				f"{doctype}: {nts.bold(import_path)} must be a subclass of {original_class_path}",
...
def _get_extended_class(base_class, doctype):
	...
	extensions = nts.get_hooks("extend_doctype_class", {}).get(doctype)
	if not extensions:
		return base_class

	# Get extension classes in reverse order using nts.get_attr
	extension_classes = []
	for extension_path in reversed(extensions):
		try:
			extension_class = nts.get_attr(extension_path)
...
class BaseDocument:
	...
	def _init_child(self, value, key):
		...
		if not __dict.get("name"):
			__dict["__islocal"] = 1
			__dict["__temporary_name"] = nts.generate_hash(length=10)
	...
	def get_valid_columns(self) -> list[str]:
		valid_columns_cache = nts.local.valid_columns
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		try:
			...
			if (
				nts.db.db_type == "postgres" and self.meta.autoname == "hash" and not name
			...
			if nts.db.is_data_too_long(e):
				...
				if column:
					...
					nts.throw(
						...
						).format(nts.bold(label), nts.bold(self.doctype)),
	...
	def get_invalid_links(self, is_submittable=False):
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if df.fieldtype == "Link":
				...
				if not doctype:
					nts.throw(_("{0} must be set first").format(_(self.meta.get_label(df.options))))
			...
			check_docstatus = is_submittable and nts.get_meta(doctype).is_submittable
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (nts.in_test and value.startswith("_T-")):
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields, mask_fields=None):
		...
		if not self.is_new() and masked_fieldnames:
			ref_values = nts.db.get_value(self.doctype, self.name, masked_fieldnames, as_dict=True) or {}
