...
from nts.model.utils.mask import mask_field_value
...
from nts.types import Filters, FilterSignature, FilterTuple
...
class DatabaseQuery:
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = nts.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			nts.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return nts.call(controller.get_list, args=kwargs, **kwargs)
	...
	def mask_fields(self, result):
		...
		from nts.model.utils.mask import mask_dict_results, mask_list_results
...
{limit}""".format(**args)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if any(re.search(r"\b" + pattern + r"\b", sanitized) for pattern in subquery_indicators):
			nts.throw(_("Cannot use sub-query here."))
		...
		for field in parameters.split(","):
			...
			for func in blacklisted_sql_functions:
				if re.search(r"\b" + re.escape(func) + r"\W*\(", field.lower()):
					nts.throw(_("Cannot use {0} in order/group by").format(field))
