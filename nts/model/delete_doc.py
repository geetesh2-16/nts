...
def delete_doc(
	...
	Args:
		doctype (str, optional): The document type to delete. If not provided,
			retrieved from nts.form_dict.get("dt"). Defaults to None.
		name (str | int | list, optional): The name/ID of the document(s) to delete.
			...
			retrieved from nts.form_dict.get("dn"). Defaults to None.
		...
		ignore_missing (bool, optional): When True, doesn't raise an error if the
			...
			nts.DoesNotExistError if document is missing. Defaults to True.
	...
	Raises:
		nts.DoesNotExistError: When document doesn't exist and ignore_missing=False.
		nts.LinkExistsError: When document is linked to other records and force=False.
		nts.PermissionError: When user doesn't have delete permissions and ignore_permissions=False.
		nts.ValidationError: When trying to delete a submitted document.
		nts.QueryTimeoutError: When document is locked by another user.
...
def check_permission_and_not_submitted(doc):
	# permission
	if not doc.flags.ignore_permissions and nts.session.user != "Administrator":
		if doc.doctype == "DocType" and not doc.custom:
			nts.throw(_("Only the Administrator can delete a standard DocType."))
		else:
	...
	if doc.meta.is_submittable and doc.docstatus.is_submitted():
		nts.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.nts.io/erpnext/user/manual/en/delete-submitted-document' target='_blank'>",
				"</a>",
			),
