...
from nts.database.utils import commit_after_response
...
from nts.model.base_document import BaseDocument, D, get_controller
from nts.model.docstatus import DocStatus
from nts.model.naming import set_new_name, validate_name
from nts.model.utils import is_virtual_doctype, simple_singledispatch
from nts.model.workflow import set_workflow_state_on_action, validate_workflow
from nts.types import DF
from nts.types.filter import FilterSignature
from nts.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `nts.model.Document` object.
...
class Document(BaseDocument):
	...
	def load_from_db(self) -> "Self":
		...
		if not is_doctype and self.meta.issingle:
			...
			if not is_doctype and isinstance(self.name, str | int):
				for_update = ""
				if self.flags.for_update and nts.db.db_type != "sqlite":
	...
	def mask_fields(self):
		from nts.model.db_query import mask_field_value

		mask_fields = nts.get_meta(self.doctype).get_masked_fields()
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = nts.in_test if ignore_version is None else ignore_version
	...
	def update_single(self, d):
		...
		if self.doctype in nts.db.value_cache:
			nts.db.value_cache.pop(self.doctype, None)
				...
				if fail:
					nts.throw(
						_("Value cannot be changed for {0}").format(
							nts.bold(_(self.meta.get_label(field.fieldname)))
						),
	...
	def run_notifications(self, method):
		...
		if (
			method == "onload"
			or (nts.flags.in_import and nts.flags.mute_emails)
		...
		notifications = nts.client_cache.get_value(
	...
	def discard(self):
		...
		if not self.docstatus.is_draft():
			raise nts.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def run_post_save_methods(self):
		...
		if not (nts.flags.in_import and self.is_new()):
			self.clear_cache()
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						nts.db._disable_transaction_control += 1
...
def bulk_insert(
	...
	while document_batch := list(itertools.islice(documents, chunk_size)):
		...
		for dt, docs in values_map.items():
			nts.db.bulk_insert(dt, valid_column_map[dt], docs, ignore_duplicates=ignore_duplicates)
...
def unlock_document(doctype: str, name: str):
	nts.get_lazy_doc(doctype, name).unlock()
	nts.msgprint(nts._("Document Unlocked"), alert=True)


def get_lazy_controller(doctype):
	lazy_controllers = nts.lazy_controllers.setdefault(nts.local.site, {})
...
def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `nts.get_doc`, but return from cache if available."""
	if (key := can_cache_doc(args)) and (doc := nts.cache.get_value(key)):
		return doc
...
def _set_document_in_cache(key: str, doc: "Document") -> None:
	nts.cache.set_value(key, doc, expires_in_sec=3600)
...
def clear_document_cache(doctype: str, name: str | None = None) -> None:
	nts.db.value_cache.pop(doctype, None)

	def clear_in_redis():
		if name is not None:
			nts.cache.delete_value(get_document_cache_key(doctype, name))
		else:
			nts.cache.delete_keys(get_document_cache_key(doctype, ""))

	clear_in_redis()
	if hasattr(nts.db, "after_commit"):
		nts.db.after_commit.add(clear_in_redis)
		nts.db.after_rollback.add(clear_in_redis)

	if doctype == "System Settings" and hasattr(nts.local, "system_settings"):
		delattr(nts.local, "system_settings")

	if doctype == "Website Settings" and hasattr(nts.local, "website_settings"):
		delattr(nts.local, "website_settings")
...
def get_cached_value(
	...
	if isinstance(fieldname, str):
		if as_dict:
			nts.throw("Cannot make dict for single fieldname")
		return doc.get(fieldname)

	values = [doc.get(f) for f in fieldname]
	if as_dict:
		return nts._dict(zip(fieldname, values, strict=False))
	return values
...
def get_single_value(setting: str, fieldname: str, /, *, as_dict: bool = False):
	...
	Usage:
		telemetry_enabled = nts.get_single_value("System Settings", "telemetry_enabled")
	"""
...
def get_last_doc(
	...
	*,
	...
	d = nts.get_all(doctype, filters=filters, limit_page_length=1, order_by=order_by, pluck="name")
	if d:
