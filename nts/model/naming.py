...
class InvalidUUIDValue(nts.ValidationError):
	pass
...
class NamingSeries:
	...
	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(nts.db.get_value("Series", prefix, "current", order_by="name", for_update=True))
...
def set_new_name(doc):
	...
	if autoname.lower() not in ("prompt", "uuid") and not nts.flags.in_import:
		doc.name = None
	...
	if meta.autoname == "UUID":
		if not doc.name:
			...
			try:
				...
				nts.throw(_("Invalid value specified for UUID: {}").format(doc.name), InvalidUUIDValue)
		return
...
def _set_amended_name(doc):
	...
	if not amend_naming_rule:
		amend_naming_rule = nts.get_single_value("Document Naming Settings", "default_amend_naming")
