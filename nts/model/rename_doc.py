...
def rename_doc(
	...
	if merge:
		nts.delete_doc(doctype, old, ignore_permissions=ignore_permissions)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	...
	for user in common_assignments:
		...
		for todo in todos:
			nts.delete_doc("ToDo", todo.name, force=True)
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			...
			nts.db.set_value(parent, {docfield: cstr(old)}, docfield, cstr(new), update_modified=False)
