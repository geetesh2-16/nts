"""
Traced Fields for nts

This module provides utilities for creating traced fields in nts documents,
...
- Seamlessly integrate with nts's document model

Example of standard usage:
		from nts.model.trace import TracedDocument, traced_field
...
See nts.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in nts documents.

	This class allows for monitoring and validating changes to specific fields
	in a nts document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if nts.in_test:
				nts.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				nts.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if nts.in_test:
					nts.throw(str(e), AssertionError)
				else:
					nts.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for nts documents with traced fields.

	This class extends nts's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from nts.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in nts documents.
...
from nts.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
