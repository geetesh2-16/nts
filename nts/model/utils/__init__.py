...
def is_virtual_doctype(doctype: str):
	if nts.flags.in_install or nts.flags.in_migrate:
		return (
			nts.db.get_value("DocType", doctype, "is_virtual")
			if nts.db.has_column("DocType", "is_virtual")
			else False
		)

	return getattr(nts.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if nts.flags.in_install or nts.flags.in_migrate:
		...
		return getattr(nts.get_meta(doctype), "issingle", False)
