...
def evaluate_workflow_value(value, evaluate_as_expression, doc):
	...
	if evaluate_as_expression:
		try:
			return nts.safe_eval(value, get_workflow_safe_globals(), dict(doc=doc.as_dict()))
...
def apply_workflow(doc, action):
	...
	if transition.transition_tasks:
		workflow_transitions = nts.db.get_all(
		...
		workflow_methods = [{"name": "Create a customer", "method":
					 		"nts.dotted.path.create_customer"}]
		"""

		tasks = {i["name"]: i["method"] for i in nts.get_hooks("workflow_methods")}
		...
		for workflow_transition in workflow_transitions:
			...
			if workflow_transition.task in DEFAULT_WORKFLOW_TASKS:
				match workflow_transition.task:
					case "Webhook":
						webhook = nts.get_doc("Webhook", workflow_transition.link)
						task_method = webhook.execute_for_doc

					case "Server Script":
						server_script = nts.get_doc("Server Script", workflow_transition.link)
						task_method = server_script.execute_workflow_task

			else:  # normal app-defined tasks
				try:
					task_method = nts.get_attr(tasks[workflow_transition.task])
				except KeyError:
					nts.throw(_('There is no task called "{}"').format(workflow_transition.task))
		...
		for async_task in async_tasks:
			nts.enqueue(async_task, doc=doc, enqueue_after_commit=True)
