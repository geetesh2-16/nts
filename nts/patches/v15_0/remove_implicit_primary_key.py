	...
	for doctype in possible_log_types:
		if (
			...
			if nts.db.db_type == "mariadb":
				nts.db.sql(f"ALTER TABLE `tab{doctype}` MODIFY name varchar({nts.db.VARCHAR_LEN})")
			else:
				nts.db.sql(
					f"ALTER TABLE `tab{doctype}` ALTER COLUMN name TYPE varchar({nts.db.VARCHAR_LEN}) USING name::varchar"
				)
