		...
		if (!nts.route_options || !nts.route_options.frm) {
			nts.model.with_doc(doctype, docname, () => {
				...
				nts.model.with_doctype(doctype, () => {
					...
					frm.meta.module && nts.app.sidebar.show_sidebar_for_module(frm.meta.module);
					print_view.show(frm);
				});
			});
		} else {
			...
			meta.module && nts.app.sidebar.show_sidebar_for_module(meta.module);
...
nts.ui.form.PrintView = class {
	...
	show(frm) {
		...
		// print designer link
		if (!cint(nts.boot.sysdefaults.disable_product_suggestion)) {
	...
	edit_print_format() {
		...
		if (is_standard_jinja_custom) {
			let doc = nts.get_doc("Print Format", print_format.name);
			nts.model.with_doctype("Print Format", () => {
				let newdoc = nts.model.copy_doc(doc);
				nts.set_route("Form", "Print Format", newdoc.name);
			});
			return;
		}
	...
	setup_print_format_dom(out, $print_format) {
		...
		setTimeout(() => {
			...
			// since nts's default shortcuts don't work within iframes.
