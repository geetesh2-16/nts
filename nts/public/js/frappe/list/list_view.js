...
nts.views.ListView = class ListView extends nts.views.BaseList {
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (allowed_filter_types.includes(f[2]) && nts.model.is_non_std_field(f[1])) {
				const df = nts.meta.get_field(doctype, f[1]);
				if (df && !df.read_only) {
					options[f[1]] = f[3];
				}
			}
	...
	get_left_html(doc) {
		...
		for (let i = 0; i < this.columns.length; i++) {
			...
			if (nts.is_mobile() && col.type == "Field" && [3, 4].includes(i)) {
	...
	get_column_html(col, doc, show_in_mobile) {
		...
		const field_html = () => {
			...
			let masked_fields = nts.get_meta(this.doctype).masked_fields || [];
		...
		if (nts.is_mobile() && col.type == "Subject") {
			css_class += " bold";
		}
	...
	apply_column_widths() {
		...
		Object.entries(this.column_max_widths).forEach(([fieldname, width]) => {
			$(`.list-view .nts-list .result .level-left .list-row-col.${fieldname}`).css({
				width: width,
				flex: `1 0 ${width}px`,
			});
		});
	...
	update_listview_classes(has_assignto, assign_to_count) {
		...
		// disable scrolling
		if (this.list_view_settings?.disable_scrolling && !nts.is_mobile()) {
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${
						nts.avatar_group(assigned_users, this.max_number_of_avatars - 1, {
							filterable: true,
						})[0].outerHTML
					}
				</div>`;
	...
	get_menu_items() {
		...
		if (nts.user.has_role("System Manager") && nts.boot.developer_mode) {
			// edit doctype
			items.push({
	...
	make_group_by_fields_modal() {
		...
		d.set_primary_action(__("Save"), ({ group_by_fields }) => {
			...
			nts.msgprint(__("Saving Changes..."));
			setTimeout(() => {
				location.reload();
			}, 1500);
		});
	...
	get_group_by_dropdown_fields() {
		let group_by_fields = [];
		let default_fields = nts.get_user_settings(this.doctype)?.group_by_fields || [];
	...
	get_actions_menu_items() {
				...
				action: () => {
					...
					nts.meta.get_docfields(doctype).forEach((field_doc) => {
						...
						if (field_doc.fieldtype === "Table" && field_doc.options) {
							...
							const child_fields = nts.meta.get_docfields(child_doctype);
		...
		const copy_to_clipboard = () => {
			return {
				...
				action: () => {
						...
						return columns
							.map((col) => {
								...
								if (col.type === "Status" || col.fieldname === "docstatus") {
									...
									const indicator = nts.get_indicator(item, this.doctype);
									...
									if (link_title_fieldname) {
										...
										if (
											...
											let link_title = nts.utils.get_link_title(
												df.options,
												item[col.fieldname]
											);
					...
					nts.utils.copy_to_clipboard(clipboard_data, message);
				},
				standard: true,
			};
		};
		...
		// bulk delete
		if (nts.model.can_delete(doctype) && is_bulk_edit_allowed(doctype)) {
