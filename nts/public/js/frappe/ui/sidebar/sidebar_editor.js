export class SidebarEditor {
	...
	handle_route_change() {
		...
		nts.router.on("change", function () {
			if (nts.get_prev_route().length == 0) return;
			if (nts.get_prev_route().length !== nts.get_route().length && me.edit_mode) {
				me.stop();
			}
		});
	}
	...
	setup_editing_controls() {
		...
		this.save_sidebar_button.on("click", async function (event) {
			...
			await nts.call({
				type: "POST",
				method: "nts.desk.doctype.workspace_sidebar.workspace_sidebar.add_sidebar_items",
				args: {
					sidebar_title: me.workspace_title || me.sidebar.sidebar_title,
					sidebar_items: me.new_sidebar_items,
				},
				callback: function (r) {
					nts.boot.workspace_sidebar_item[r.message.name.toLowerCase()] = [
	...
	get_item_obj(item_data) {
		return nts.app.sidebar.items.find((item) => {
	...
	make_dialog(opts) {
		...
		let d;
		this.dialog = d = new nts.ui.Dialog({
			title: title,
