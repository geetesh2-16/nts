nts.ui.SidebarHeader = class SidebarHeader {
	constructor(sidebar) {
		...
		this.dropdown_items = [
			...
			},
			{
				...
				icon: "edit",
				condition: function () {
					return nts.boot.developer_mode;
				},
				onClick: function () {
		...
		if (nts.boot.desk_settings.notifications) {
			let is_dark = nts.ui.get_current_theme() === "dark";
	...
	fetch_related_icons() {
		...
		if (nts.current_app) {
			let desktop_icons = [...nts.boot.desktop_icons];
			desktop_icons.splice(
				desktop_icons.indexOf(nts.utils.get_desktop_icon_by_label(this.title)),
				1
			);
			...
			sibling_icons.forEach((icon) => {
				...
				if (!workspaces_not_to_show.includes(icon.label)) {
					let item = {
						...
						label: icon.label,
						url: nts.utils.get_route_for_icon(icon),
					};
					...
					if (
						nts.utils.get_desktop_icon(icon.label, nts.boot.desktop_icon_style)
					) {
						item.icon_url = nts.utils.get_desktop_icon(
							icon.label,
							nts.boot.desktop_icon_style
						);
					} else {
						item.icon_html = nts.utils.desktop_icon(icon.label, "gray", "sm");
					}
	...
	get_icon_for_menu_item(icon, item) {
		if (nts.utils.get_desktop_icon(icon.label, nts.boot.desktop_icon_style)) {
			item.icon_url = nts.utils.get_desktop_icon(
				icon.label,
				nts.boot.desktop_icon_style
			);
	...
	build_folder_map(desktop_icons) {
		const folder_map = {};
		const sibling_icons = [];
		if (!nts.current_app) return;
		this.sort_icons(desktop_icons);
		desktop_icons.forEach((icon) => {
			if (
				icon.link_type != "External" &&
				icon.app == nts.current_app.app_name &&
				!icon.hidden
			) {
				...
				if (icon.parent_icon) {
					icon.url = nts.utils.get_route_for_icon(icon);
	...
	get_help_siblings() {
		const navbar_settings = nts.boot.navbar_settings;
	...
	get_custom_help_links() {
		...
		for (let i = 0; i < breadcrumbs.length; i++) {
			...
			let key = r.join("/");
			let help_links = nts.help.help_links[key] || [];
	...
	make() {
		...
		$(
			nts.render_template("sidebar_header", {
	...
	set_header_icon() {
		...
		let desktop_icon_url =
			desktop_icon && nts.utils.get_desktop_icon(desktop_icon.label, "solid");
		if (desktop_icon_url) {
			...
			this.header_icon = nts.utils.desktop_icon(this.sidebar.sidebar_title, "gray", "sm");
		} else {
	...
	get_default_icon() {
		return nts.boot.app_data[0].app_logo_url;
	}
	get_desktop_icon_by_label(title, filters) {
		if (!filters) {
			return nts.boot.desktop_icons.find((f) => f.label === title && f.hidden != 1);
		} else {
			return nts.boot.desktop_icons.find((f) => {
				return (
					f.label === title &&
