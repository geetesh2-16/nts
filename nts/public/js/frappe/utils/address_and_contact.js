	...
	render_address_and_contact: function (frm) {
		...
		for (const item of items) {
			...
			if (field_wrapper && frm.doc.__onload && item.data in frm.doc.__onload) {
				$(field_wrapper)
					.html(nts.render_template(item.template, frm.doc.__onload))
					.find(item.btn)
...
function new_record(doctype, frm) {
	...
	if (nts.boot.enable_address_autocompletion === 1 && doctype === "Address") {
		new nts.ui.AddressAutocompleteDialog({
