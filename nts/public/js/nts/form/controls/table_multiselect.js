nts.ui.form.ControlTableMultiSelect = class ControlTableMultiSelect extends (
		...
		this.$input_area.on("click", ".btn-remove", (e) => {
			...
			const rows = this._get_rows().filter((row) => {
				...
				nts.run_serially([
					...
					() => {
						nts.model.clear_doc(this.df.options, row.name);
	...
	parse(value) {
		...
		let new_row;
		if (this.frm) {
			new_row = nts.model.add_child(this.frm.doc, this.df.options, this.df.fieldname);
	...
	async validate(value) {
		...
		if (!this.df.ignore_link_validation) {
			...
			if (nts.utils.is_empty(validated_value)) {
				return all_rows_except_last;
			}
	...
	get_pill_html(value) {
		...
		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				<span class="btn-link-to-form">${__(nts.utils.escape_html(pill_name))}</span>
