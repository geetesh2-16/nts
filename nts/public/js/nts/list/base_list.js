...
nts.views.BaseList = class BaseList {
	 ...
	 * to the `nts_list` container. This container is used to create a scrollable area for the result content.
	 */
	setup_result_container_area() {
		if (this.view == "List") {
			this.$nts_list.append($(`<div class="result-container">`));
		}
	}

	setup_result_area() {
		this.$result = $(`<div class="result">`);
		let nts_list = this.$nts_list;
		if (this.view == "List") {
			nts_list = this.$nts_list.find(".result-container");
		}
		nts_list.append(this.$result);
	}
	...
	set_result_height() {
		...
		this.$result.parent(".result-container").css({
			height: resultContainerHeight - (nts.is_mobile() ? 100 : 0) + "px",
		});
...
class FilterArea {
	constructor(list_view) {
		...
		this.setup();
		if (nts.is_mobile()) this.setup_mobile(list_view);
	}
	...
	setup() {
		...
		this.user_setting_fields =
			nts.get_user_settings(this.list_view.doctype)?.group_by_fields || [];
	...
	render_non_standard_fields_filter() {
		let get_item_html = (fieldname) => {
			...
			return `<div class="group-by-field list-link form-group nts-control input-max-width">
	...
	get_stats($dropdown) {
		...
		nts.call({
			...
			callback: function (r) {
				...
				nts.utils.setup_search($dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	filter_field_with_match_type(df) {
		setTimeout(() => {
			...
			const getIcon = (match_type) => {
				if (match_type === "=") {
					return nts.utils.icon("equal");
				} else {
					return nts.utils.icon("equal-approximately");
				}
			};
