...
nts.search.AwesomeBar = class AwesomeBar {
	...
	setup_search_modal(element) {
		...
		let search_modal = new nts.get_modal("Search", "");
		...
		let search_modal_body = `<div class="align-baseline flex py-2 px-1 relative navbar-modal-wrapper">
			<div class="modal-search-icon absolute pr-2 pl-2">${nts.utils.icon("search")}</div>
			<input
				id="navbar-search"
				type="text"
		...
		let search_modal_footer = `<div class="awesomebar-modal-footer flex justify-between w-100">
			<div class="help-navigation">
				<span class="help-item-navigate">
					<span class="help-item">${nts.utils.icon("arrow-up")}</span>
					<span class="help-item">${nts.utils.icon("arrow-down")}</span>
					<span>${__("to navigate")}</span>
				</span>
				<span class="help-item-navigate">
					<span class="help-item">${nts.utils.icon("corner-down-left")}</span>
					<span>${__("to select")}</span>
				</span>
				<span class="help-item help-item-esc">${__("esc")}</span>
				<span>${__("to close")}</span>
			</div>
			<div class="pointer">${nts.utils.icon("circle-question-mark")}</div>
		</div>`;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				if (d.type == "Desktop Icon") {
					target = nts.utils.get_route_for_icon(d.icon_data);
					d.route = target;
					d.route_options = {
	...
	show_help() {
		const txt =
			...
			__("Open in new tab") +
			"</td><td>" +
			(nts.utils.is_mac() ? "⌘ + Enter" : "Ctrl + Enter") +
			"</td></tr>\
			<tr><td>" +
			__("Calculate") +
			"</td><td>" +
	...
	build_options(txt) {
		var options = nts.search.utils
			...
			.concat(
				...
				nts.search.utils.get_desktop_icons(txt),
	...
	make_global_search(txt) {
		...
		this.options.push({
			...
			`,
			value: __("Search for {0}", [nts.utils.xss_sanitise(txt)]),
			match: txt,
			index: 100,
			default: "Search",
			onclick: function () {
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			this.options.push({
				...
				value: __("Find {0} in {1}", [nts.utils.xss_sanitise(txt), __(route[1])]),
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				var formatted_value = __("{0} = {1}", [
					nts.utils.xss_sanitise(txt),
					(rounded_val + "").bold(),
				]);
				this.options.push({
					label: formatted_value,
					value: __("{0} = {1}", [nts.utils.xss_sanitise(txt), rounded_val]),
					match: rounded_val,
					index: 80,
					default: "Calculator",
					onclick: function () {
