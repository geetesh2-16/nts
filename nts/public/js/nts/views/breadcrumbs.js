...
nts.breadcrumbs = {
	...
	update() {
		...
		if (breadcrumbs.type === "Custom") {
			...
			if (breadcrumbs.doctype && ["print", "form"].includes(view)) {
				...
				breadcrumbs.label = nts.query_report.page_title;
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (nts.app.sidebar.sidebar_title) {
			let icon = nts.utils.get_desktop_icon_by_label(nts.app.sidebar.sidebar_title);
			let url = nts.utils.get_route_for_icon(icon);
			if (url) {
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			const doctype_meta = nts.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				if (
					...
					breadcrumbs.workspace =
						nts.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = nts.get_meta(doctype);
		if (
	...
	set_form_breadcrumb(breadcrumbs, view) {
		...
		let doc = nts.get_doc(doctype, docname);
		let form_route = `/desk/${nts.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (docname.startsWith("new-" + doctype.toLowerCase().replace(/ /g, "-"))) {
			...
			let title = nts.model.get_doc_title(doc);
			docname_title = title || doc.name;
			if (nts.utils.is_html(docname_title)) {
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		...
		let dashboard_route = `/desk/${nts.router.slug(doctype)}/${docname}`;
	...
	clear() {
		...
		this.append_breadcrumb_element("/desk", nts.utils.icon("monitor"));
	},
