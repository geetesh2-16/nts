...
nts.views.Calendar = class Calendar {
	...
	make() {
		...
		this.fullCalendar = new nts.FullCalendar(this.$cal[0], this.cal_options);
	...
	get_system_datetime(date) {
		return nts.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: nts.FullCalendar.Plugins,
			...
			eventClick: function (info) {
				...
				if (nts.model.can_read(doctype)) {
					nts.set_route("Form", doctype, info.event.id);
				}
			},
	...
	update_event(event, revertFunc) {
		var me = this;
		nts.model.remove_from_locals(me.doctype, event.id);
		return nts.call({
