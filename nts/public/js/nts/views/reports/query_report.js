...
nts.views.QueryReport = class QueryReport extends nts.views.BaseList {
		...
		return nts.model
			...
			.then(
				() =>
					...
					nts.app.sidebar.show_sidebar_for_module(this.report_doc.module)
			);
	}
	...
	add_collapse_button() {
		...
		if (this.filters[filter_no]) {
			this.$collapse_button = $(`<div>${nts.utils.icon("chevron-down", "md")}</div>`);
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			const js_filters = (nts.query_reports[this.report_name]?.filters || [])
				.filter(
	...
	update_masked_fields_in_columns(columns) {
		const masked_fields = nts.get_meta(this.report_doc?.ref_doctype).masked_fields;
	...
	async get_report_print_format(report_name) {
		...
		const r = await nts.db.get_value("Print Format", filters, ["html", "css"]);
		if (r && r.message && r.message.html) {
			const css = r.message.css || "";
			const html = r.message.html || "";
			return `<style>${css}</style>${html}`;
		} else {
			nts.msgprint(__("Print Format not found"));
			return null;
		}
	}
	...
	async render_report_letterhead(print_settings) {
		...
		if (!doc_context.company) {
			doc_context.company = nts.defaults.get_default("company");
		}

		try {
			const r = await nts.call("nts.utils.print_format.render_letterhead_for_print", {
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				...
				if (nts.utils.is_empty(value) || docfield.hidden_due_to_dependency) {
					return null;
				}
				...
				if (docfield.fieldtype === "Check") {
					...
					display_value = nts.format(value, docfield);
				}
				...
				extra_fields.push(
					...
					},
					{
						...
						description: __("Hidden columns include: <br> {0}", [
							nts.utils.comma_and(Array.from(hidden_fields)),
						]),
					}
				);
			}
