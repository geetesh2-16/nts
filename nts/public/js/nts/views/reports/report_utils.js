...
nts.report_utils = {
	make_chart_options: function (
		...
		function get_translated_column_label(fieldname) {
			...
			return column?.label ?? __(nts.model.unscrub(fieldname));
		}
	},
	...
	get_csv_preview(data, quoting, delimiter, decimal_sep) {
		...
		if (decimal_sep.length > 1) {
			nts.throw(__("Decimal Separator must be a single character"));
		}
		...
		if (decimal_sep !== "." && quoting === QUOTING.NonNumeric) {
			nts.throw(__("Decimal Separator must be '.' when Quoting is set to Non-numeric"));
		}
