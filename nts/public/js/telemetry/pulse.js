class PulseProvider {
	...
	is_enabled() {
		return nts.boot.telemetry_provider?.includes("pulse") && nts.boot.enable_telemetry;
	}
	...
	sendEvents(events) {
		...
		return new Promise((resolve, reject) => {
			try {
				nts.call({
					method: "nts.utils.telemetry.pulse.client.bulk_capture",
					args: { events },
					type: "POST",
					no_spinner: true,
					freeze: false,
	...
	sendBeacon(events) {
		try {
			if (navigator.sendBeacon) {
				const url = "/api/method/nts.utils.telemetry.pulse.client.bulk_capture";
