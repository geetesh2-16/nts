...
class PushNotification:
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		secret = nts.generate_hash(length=32)

		# store the token in the redis cache
		nts.cache.set_value(f"push_relay_registration_token:{secret}", token, expires_in_sec=600)
		body = {
			"endpoint": self._site_name,
			"protocol": self._site_protocol,
			"port": self._site_port,
			"token": token,
			"webhook_route": f"/api/method/nts.push_notification.auth_webhook?secret={secret}",
		}
...
def auth_webhook(secret: str):
	...
	if token := nts.cache.get_value(f"push_relay_registration_token:{secret}"):
		response.data = token
