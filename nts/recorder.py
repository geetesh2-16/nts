...
import nts
from nts import _
from nts.database.utils import is_query_type
...
def record(force=False):
	flag_value = nts.client_cache.get_value(RECORDER_INTERCEPT_FLAG)
	if flag_value or force:
		...
		nts.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
...
def start(
	...
	**kwargs,
	...
	nts.client_cache.set_value(RECORDER_INTERCEPT_FLAG, True)
	nts.cache.expire_key(RECORDER_INTERCEPT_FLAG, RECORDER_AUTO_DISABLE)
...
def stop(*args, **kwargs):
	nts.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
	nts.enqueue(post_process, now=nts.in_test)
