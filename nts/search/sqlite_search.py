...
class SQLiteSearch(ABC):
	"""
	Abstract base class for SQLite FTS5-based full-text search for nts.
	...
	def search(self, query, title_only=False, filters=None):
		...
		try:
			...
			nts.log_error(f"Search query failed: {e}")
			raw_results = []
			total_matches = 0
	...
	def drop_index(self):
		...
		if os.path.exists(self.db_path):
			try:
				...
				nts.log_error(f"Failed to remove search index file {self.db_path}: {e}")
				raise
	...
	def _get_connection(self, read_only=False):
		...
		try:
			...
			nts.log_error(f"Failed to connect to search database: {e}")
	...
	def _update_progress(self, message, progress, total=100, absolute=True):
		...
		if not hasattr(nts.local, "request") and not nts.flags.in_test:
			...
			if field == "modified":
				...
				if modified_value:
					if not isinstance(modified_value, datetime.datetime):
						modified_value = nts.utils.get_datetime(modified_value)
...
def build_index(
	...
	if search_class_path:
		SearchClass = nts.get_attr(search_class_path)
...
def build_index_in_background():
	...
	for SearchClass in search_classes:
		...
		nts.enqueue(
			"nts.search.sqlite_search.build_index",
			queue="long",
...
def get_search_classes() -> list[type[SQLiteSearch]]:
	module_paths = nts.get_hooks("sqlite_search")
	search_classes = [nts.get_attr(path) for path in module_paths]
