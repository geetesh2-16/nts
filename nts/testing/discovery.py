"""
This module provides functionality for discovering and organizing tests in the nts framework.
...
logger = logging.getLogger("nts.testing.discovery")
...
def discover_doctype_tests(doctypes: list[str], runner, app: str, force: bool = False) -> "TestRunner":
	...
	for doctype in doctypes:
		try:
			...
			doctype_app = nts.db.get_value("Module Def", module, "app_name")
			if app is None:
				_app = doctype_app
			...
			test_module = nts.modules.utils.get_module_name(doctype, module, "test_")
			force and nts.db.delete(doctype)
	...
	for test in runner._iterate_suite(test_suite):
		...
		match test:
			...
			case _:
				...
				if any(b.__name__ == "ntsTestCase" for b in test.__class__.__bases__):
					from nts.deprecation_dumpster import deprecation_warning

					deprecation_warning(
						"2024-20-08",
						"v17",
						"accurate categorization of ntsTestCase will be removed from this runner",
					)
					category = "old-nts-test-class-category"
