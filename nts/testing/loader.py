...
import nts
from nts.modules.utils import get_module_name
from nts.testing.config import TestParameters


class ntsTestLoader(unittest.TestLoader):
	...
	def discover_tests(self, params: TestParameters) -> unittest.TestSuite:
		...
		if self.params.tests:
			...
			files = self.get_files(nts.get_installed_apps())
			...
			module = nts.get_cached_value("DocType", self.params.doctype, "module")
			app = nts.get_cached_value("Module Def", module, "app_name")
