"""
This module contains the TestRunner class, which is responsible for executing test suites in nts.
...
- Integration with nts's configuration and environment setup
...
Usage:
The TestRunner is typically instantiated and used by nts's test discovery and execution system.
...
class TestRunner(unittest.TextTestRunner):
	...
	def _prepare_category(self, category, suite, app):
		from nts.deprecation_dumpster import get_compat_nts_test_case_preparation

		dispatcher = {
			"integration": self.integration_preparation,
			"old-nts-test-class-category": get_compat_nts_test_case_preparation(self.cfg),
