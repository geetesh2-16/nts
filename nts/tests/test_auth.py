...
class TestAllowedReferrer(UnitTestCase):
	def test_is_allowed_referrer(self):
		...
		nts.cache.set_value("allowed_referrers", ["https://example.com"])

		# Test with valid referrer
		nts.local.request = create_request({"Referer": "https://example.com/some/path"})
		http_request = nts.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid referrer
		nts.local.request = create_request({"Referer": "https://malicious.com"})
		http_request = nts.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test with valid origin
		nts.local.request = create_request({"Origin": "https://example.com"})
		http_request = nts.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid origin
		nts.local.request = create_request({"Origin": "https://malicious.com"})
		http_request = nts.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test subdomain bypass prevention
		nts.local.request = create_request({"Referer": "https://example.com.evil.com"})
		http_request = nts.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test exact domain match for referrer
		nts.local.request = create_request({"Referer": "https://example.com"})
		http_request = nts.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Clean up
		nts.cache.delete_value("allowed_referrers")
...
class TestSessionExpiry(ntsAPITestCase):
