...
class TestDBQuery(IntegrationTestCase):
	...
	def test_string_as_field(self):
		self.assertEqual(
			nts.get_all("DocType", as_list=True), nts.get_all("DocType", fields="name", as_list=True)
		)
	...
	def test_query_fields_sanitizer(self):
		...
		from nts.desk.search import search_link
	...
	def test_virtual_doctype(self):
		...
		class VirtualDocType:
			...
			def get_list(args=None, limit_page_length=0, doctype=None):
				from nts.types.filter import FilterTuple
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("IF", nts.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0).get_sql())
		self.assertIn("IFNULL", nts.get_all("User", {"first_name": ("in", ["a", None])}, run=0).get_sql())
		self.assertIn("IFNULL", nts.get_all("User", {"first_name": ("in", ["a", ""])}, run=0).get_sql())
		self.assertIn("IFNULL", nts.get_all("User", {"first_name": ("in", [])}, run=0).get_sql())
		self.assertIn("IFNULL", nts.get_all("User", {"first_name": ("not in", ["a"])}, run=0).get_sql())
		self.assertIn("IFNULL", nts.get_all("User", {"first_name": ("not in", [])}, run=0).get_sql())
		self.assertIn("IFNULL", nts.get_all("User", {"first_name": ("not in", [""])}, run=0).get_sql())

		# primary key is never nullable
		self.assertNotIn("IFNULL", nts.get_all("User", {"name": ("in", ["a", None])}, run=0).get_sql())
		self.assertNotIn("IFNULL", nts.get_all("User", {"name": ("in", ["a", ""])}, run=0).get_sql())
		self.assertNotIn("IFNULL", nts.get_all("User", {"name": ("in", (""))}, run=0).get_sql())
		self.assertNotIn("IFNULL", nts.get_all("User", {"name": ("in", ())}, run=0).get_sql())

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn(
			"IFNULL", nts.get_all("User", {"last_active": (">", "2022-01-01")}, run=0).get_sql()
		)
		self.assertNotIn("IFNULL", nts.get_all("User", {"creation": ("<", "2022-01-01")}, run=0).get_sql())
		self.assertNotIn(
			"IFNULL",
			nts.get_all(
		...
		self.assertIn("IFNULL", nts.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0).get_sql())
	...
	def test_ifnull_none(self):
		query = nts.get_all("DocField", {"fieldname": None}, run=0).get_sql()
		self.assertIn("IS NULL", query)
		self.assertNotIn("\\'", query)
		self.assertNotIn("ifnull", query)
		self.assertFalse(nts.get_all("DocField", {"name": None}))
		self.assertFalse(nts.get_all("DocField", {"parent": None}))
		self.assertNotIn("0", nts.get_all("DocField", {"parent": None}, run=0).get_sql())
...
class TestReportView(IntegrationTestCase):
	...
	def test_reportview_child_table_alias(self):
		from nts.desk import reportview
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			data = nts.get_list("Test Blog Post", "published")
			self.assertTrue(len(data) > 0)
