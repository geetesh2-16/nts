...
class TestNaming(IntegrationTestCase):
	...
	def test_uuid_naming(self):
		...
		self.assertEqual("uuid", nts.db.get_column_type(uuid_doctype, "name"))

		# Auto set names
		document = nts.new_doc(uuid_doctype).insert()
		...
		for uid in (uuid.uuid4(), uuid.uuid7()):
			doc = nts.new_doc(uuid_doctype, name=uid).insert()
			self.assertEqual(doc.name, str(uid))

		# Can specify valid UUID strings too
		for uid in (uuid.uuid4(), uuid.uuid7()):
			doc = nts.new_doc(uuid_doctype, name=str(uid)).insert()
			self.assertEqual(doc.name, str(uid))

		self.assertRaises(InvalidUUIDValue, nts.new_doc(uuid_doctype, name="XYZ").insert)
