...
import nts
from nts.search.sqlite_search import SQLiteSearch, SQLiteSearchIndexMissingError
...
class TestSQLiteSearchAPI(IntegrationTestCase):
	...
	def setUp(self):
		...
		for data in note_data:
			note = nts.get_doc({"doctype": "Note", "title": data["title"], "content": data["content"]})
			note.insert()
		...
		for data in todo_data:
			todo = nts.get_doc({"doctype": "ToDo", "description": data["description"], "status": "Open"})
			todo.insert()
	...
	def test_search_filtering_and_permissions(self):
		...
		original_user = nts.session.user
		try:
			# Create a test user and switch to them
			test_user_email = "test_search_user@example.com"
			if not nts.db.exists("User", test_user_email):
				test_user = nts.get_doc(
					{
						"doctype": "User",
	...
	def test_background_operations(self, mock_enqueue):
		...
		from nts.search.sqlite_search import (
			build_index_in_background,
			get_search_classes,
		)

		# Test getting search classes
		with patch("nts.get_hooks") as mock_get_hooks:
			mock_get_hooks.return_value = ["nts.tests.test_sqlite_search.TestSQLiteSearch"]
