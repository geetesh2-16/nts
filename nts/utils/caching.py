...
def request_cache(func: Callable) -> Callable:
	...
	Cache is stored in `nts.local.request_cache`.
	...
	def wrapper(*args, **kwargs):
		_cache = getattr(nts.local, "request_cache", None)
		if _cache is None:
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	The cache is stored in `nts.utils.caching._SITE_CACHE`.
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			cached_val = nts.cache.get_value(func_call_key, user=user, shared=shared)
	...
	*,
	...
	def outer(func: Callable) -> Callable:
		...
		def inner(*args, **kwargs):
			...
			if nts.request and nts.request.method == "GET" and qualified_name in nts.request.path:
				nts.local.response_headers.set("Cache-Control", cache_headers)
			return ret
...
def deprecated_local_cache(namespace, key, generator, regenerate_if_none=False):
	if namespace not in nts.local.cache:
		nts.local.cache[namespace] = {}

	if key not in nts.local.cache[namespace]:
		nts.local.cache[namespace][key] = generator()

	elif nts.local.cache[namespace][key] is None and regenerate_if_none:
		# if key exists but the previous result was None
		nts.local.cache[namespace][key] = generator()

	return nts.local.cache[namespace][key]
