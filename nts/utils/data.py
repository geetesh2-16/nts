...
from nts.locale import get_date_format, get_first_day_of_the_week, get_number_format, get_time_format
from nts.types.filter import Filters, FilterSignature, FilterTuple
from nts.utils.deprecations import deprecated
from nts.utils.number_format import NUMBER_FORMAT_MAP, NumberFormat
...
# these options are necessary to use orjson with nts
#
...
# nts converts datetime objects differently (__str__) from orjson (RFC 3339)
#
...
def get_user_date_format() -> str:
	...
	if getattr(nts.local, "user_date_format", None) is None:
		nts.local.user_date_format = get_date_format()
...
def get_user_time_format() -> str:
	...
	if getattr(nts.local, "user_time_format", None) is None:
		nts.local.user_time_format = get_time_format()

	return nts.local.user_time_format
...
from nts.deprecation_dumpster import get_number_format_info
...
def __getattr__(name):
	if name == "number_format_info":
		...
		deprecation_warning(
			"unkown",
			"v16",
			"use nts.utils.number_format.NUMBER_FORMAT_MAP instead of nts.utils.data.number_format_info",
		)
	else:
		try:
			globals()[name]
...
def money_in_words(
	...
	if not fraction_currency:
		fraction_currency = nts.db.get_value("Currency", main_currency, "fraction", cache=True)

	number_format = get_number_format()

	fraction_units = nts.db.get_value("Currency", main_currency, "fraction_units", cache=True)
	if fraction_units:
...
def get_url(
	...
	if (
		...
		not nts.conf.restart_supervisor_on_update
		and not nts.conf.restart_systemd_on_update
		and host_name
...
def get_host_name() -> str:
	...
	e.g. If site is 'https://cloud.nts.io', returns 'cloud.nts.io'.
	"""
...
def get_link_to_form(doctype: str, name: str | None = None, label: str | None = None) -> str:
	...
	e.g. get_link_to_form("Sales Invoice", "INV-0001", "Link Label") returns:
	    '<a href="https://nts.io/desk/sales-invoice/INV-0001">Link Label</a>'.
	"""
	from nts import _
...
def get_link_to_report(
	...
	e.g. get_link_to_report("Revenue Report", "Link Label") returns:
	        '<a href="https://nts.io/desk/query-report/Revenue%20Report">Link Label</a>'.
	"""
	from nts import _
...
def get_url_to_form(doctype: str, name: str | None = None) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://nts.io",
	         returns 'https://nts.io/desk/sales-invoice/INV-00001'
	"""
	if not name:
...
def get_url_to_list(doctype: str) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://nts.io",
	         returns 'https://nts.io/desk/sales-invoice'
	"""
...
def get_url_to_report(name, report_type: str | None = None, doctype: str | None = None) -> str:
	...
	e.g. when name="Sales Register" and your site URL is "https://nts.io",
	         returns 'https://nts.io/desk/query-report/Sales%20Register'

	You can optionally pass `report_type` and `doctype` to get the URL for a Report Builder report.

	get_url_to_report("Revenue", "Report Builder", "Sales Invoice") -> 'https://nts.io/desk/sales-invoice/view/report/Revenue'
	"""
...
def get_filter(doctype: str, filters: FilterSignature, filters_config=None) -> "nts._dict":
	"""Return a `_dict` like:
	...
	f = nts._dict(doctype=ft[0], fieldname=ft[1], operator=ft[2], value=ft[3])
...
def scrub_urls(html: str) -> str:
	...
	e.g. If HTML is '<a href="/files/abc.jpeg">View Image</a>' and site URL is 'https://nts.io',
	        returns '<a href="https://nts.io/files/abc.jpeg">View Image</a>'.
	"""
...
def quoted(url: str) -> str:
	...
	e.g. 'https://nts.io/files/my Image file.jpeg' -> 'https://nts.io/files/my%20Image%20file.jpeg'
	"""
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	...
	e.g. {
	        "email": "faris@nts.io",
	        "image": "/assets/nts/images/ui/avatar.png",
	        "name": "Faris Ansari"
	}
	"""
	try:
...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = nts.db.get_value("UTM Source", source, "slug") or source
		_campaign = nts.db.get_value("UTM Campaign", campaign, "slug") or campaign
		_medium = nts.db.get_value("UTM Medium", medium, "slug") or medium
	...
	source, medium, campaign = nts.cache.get_value(
...
def map_trackers(url_trackers: dict, create: bool = False):
	...
	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		source = nts.db.get_value("UTM Source", {"slug": slug(url_source)}, "name") or url_source
		if create and source == url_source and not nts.db.exists("UTM Source", source):
			source = nts.new_doc("UTM Source")
			source.name = url_source
			source.description = f"Autogenerated from {url_trackers}"
			source.save(ignore_permissions=True)
		nts_trackers["utm_source"] = source

	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		medium = nts.db.get_value("UTM Medium", {"slug": slug(url_medium)}, "name") or url_medium
		if create and medium == url_medium and not nts.db.exists("UTM Medium", medium):
			medium = nts.new_doc("UTM Medium")
			medium.name = url_medium
			medium.description = f"Autogenerated from {url_trackers}"
			medium.save(ignore_permissions=True)
		nts_trackers["utm_medium"] = medium

	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		campaign = nts.db.get_value("UTM Campaign", {"slug": slug(url_campaign)}, "name") or url_campaign
		if create and campaign == url_campaign and not nts.db.exists("UTM Campaign", campaign):
			campaign = nts.new_doc("UTM Campaign")
			campaign.name = url_campaign
			campaign.campaign_description = f"Autogenerated from {url_trackers}"
			campaign.save(ignore_permissions=True)
		nts_trackers["utm_campaign"] = campaign

	if url_content := url_trackers.get("utm_content", url_trackers.get("content")):
		nts_trackers["utm_content"] = url_content
