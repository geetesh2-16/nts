...
import nts
from nts.utils.pdf import get_host_url
from nts.utils.print_utils import convert_uom, parse_float_and_unit


class Browser:
	def __init__(self, generator, print_format, html, options):
		self.is_print_designer = nts.get_cached_value("Print Format", print_format, "print_designer")
		self.browserID = nts.utils.random_string(10)
	...
	def open(self, generator):
		from nts.utils.pdf_generator.cdp_connection import CDPSocketClient
	...
	def create_browser_context(self):
		...
		if error:
			nts.log_error(title="Error creating browser context:", message=f"{error}")
	...
	def new_page(self, page_type):
		...
		from nts.utils.pdf_generator.page import Page
	...
	def _parse_pdf_options_from_html(self):
		from nts.utils.pdf import get_print_format_styles
	...
	def prepare_options_for_pdf(self):
		...
		if not options.get("page-height") or not options.get("page-width"):
			if not (page_size := self.options.get("page-size")):
				raise nts.ValidationError("Page size is required")
			if page_size == "CUSTOM":
				raise nts.ValidationError("Custom page size requires page-height and page-width")
			size = PageSize.get(page_size)
			if not size:
				raise nts.ValidationError("Invalid page size")
	...
	def get_rendered_header_footer(self, content, type, head, styles, css):
		from nts.utils.pdf import toggle_visible_pdf

		html_id = f"{type}-html"
		content = content.extract()
		toggle_visible_pdf(content)
		id_map = {"header": "pdf_header_html", "footer": "pdf_footer_html"}
		hook_func = nts.get_hooks(id_map.get(type))
		return nts.call(
			hook_func[-1],
			soup=self.soup,
			head=head,
