...
class CDPSocketClient:
	...
	async def _connect(self):
		try:
			...
			nts.log_error(title="Failed to connect to WebSocket:", message=f"{nts.get_traceback()}")
			raise

	async def _listen(self):
		try:
			async for message in self.connection:
				self._handle_message(nts.json.loads(message))
		except Exception:
			nts.log_error(title="WebSocket listening error:", message=f"{nts.get_traceback()}")
	...
	def disconnect(self):
		try:
			...
			nts.log_error(title="Error while disconnecting:", message=f"{nts.get_traceback()}")
			raise
	...
	async def _disconnect(self):
		try:
			...
			nts.log_error(
				title="Error during WebSocket disconnection:", message=f"{nts.get_traceback()}"
			)
	...
	async def _send(self, method, params=None, session_id=None, wait_future_fulfill=True):
		...
		await self.connection.send(nts.json.dumps(message))
	...
	def wait_for_event(self, event, timeout=3):
		...
		try:
			...
			nts.log_error(title="Timeout waiting for event", message=f"{nts.get_traceback()}")
