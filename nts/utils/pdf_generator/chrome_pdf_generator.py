...
import nts
from nts import _
from nts.utils.print_utils import find_or_download_chromium_executable
...
class ChromePDFGenerator:
	...
	def _initialize_chromium(self):
		...
		site_config = nts.get_common_site_config()
		...
		if self.CHROMIUM_WEBSOCKET_URL:
			nts.warn("Using external chromium websocket url. Make sure it is accessible.")
	...
	def _verify_chromium_installation(self):
		...
		if not os.access(self._chromium_path, os.X_OK):
			nts.throw(f"Chromium not executable at {self._chromium_path}")
		return True
	...
	def start_chromium_process(self, debug=False):
		...
		try:
			if debug:
				command_args = [
					...
					"--user-data-dir=/tmp/chromium-{}-user-data".format(
						nts.local.site + nts.utils.random_string(10)
					),
					"--disable-gpu",
					"--no-sandbox",
					"--no-first-run",
			...
			nts.log_error(f"Error starting Chromium: {e}")
			nts.throw(_("Could not start Chromium. Check logs for details."))
	...
	def _close_browser(self):
		...
		"""
		if self._browsers:
			nts.log("Cannot close Chromium as there are active browser instances.")
			return
		if self._chromium_process:
			self._chromium_process.terminate()
		...
		self._chromium_process = None
		self._devtools_url = None
		nts.log("Headless Chromium closed successfully.")
	...
	def fetch_devtools_url(self, port):
		...
		try:
			...
			nts.log_error(f"An error occurred: {e}")
		return None
