...
def get_print(
	...
	for hook in nts.get_hooks("on_print_pdf"):
		nts.call(hook, doctype=doctype, name=name, print_format=print_format)
...
def attach_print(
	...
	nts.local.flags.ignore_print_permissions = True

	with print_language(lang or nts.local.lang):
		content = ""
	...
	nts.local.flags.ignore_print_permissions = False
...
def find_or_download_chromium_executable():
	...
	if chromium_path := shutil.which(nts.get_common_site_config().get("chromium_path", "")):
		return chromium_path

	bench_path = nts.utils.get_bench_path()
...
def get_chromium_download_url():
	...
	common_config = nts.get_common_site_config()
