...
from nts.permissions import AUTOMATIC_ROLES, get_rights, get_roles, get_valid_perms
...
class UserPermissions:
	...
	def setup_user(self):
		...
		if not nts.flags.in_install_db and not nts.in_test:
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = nts.get_cached_value(
...
def is_website_user(username: str | None = None) -> str | None:
	return nts.get_cached_value("User", username or nts.session.user, "user_type") == "Website User"
...
def is_portal_user():
	...
	if user_type == "Website User" and has_common(nts.get_roles(), roles):
		return True


def get_portal_roles():
	roles = []
	for menu_item in nts.get_single("Portal Settings").menu:
