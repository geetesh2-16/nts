...
class WebForm(WebsiteGenerator):
	...
	def validate(self):
		...
		in_user_env = not (
			nts.flags.in_install or nts.flags.in_patch or nts.in_test or nts.flags.in_fixtures
		)
...
"""
	...
	def get_context(self, context):
		...
		if nts.form_dict.name:
			...
			context.has_delete_permission = nts.has_permission(
				self.doc_type, "delete", nts.form_dict.name
			)
	...
	def load_translations(self, context):
		...
		context.translated_messages = nts.as_json(translation_dict)
	...
	def load_form_data(self, context):
		...
		if nts.form_dict.name:
			...
			context.title = (
				strip_html(
					nts.cstr(context.reference_doc.get(context.reference_doc.meta.get_title_field()))
				)
				or context.doc_name
			)
	...
	def has_web_form_permission(self, doctype, name, ptype="read"):
		...
		if self.apply_document_permissions:
			return nts.get_lazy_doc(doctype, name).has_permission(permtype=ptype)
...
def accept(web_form, data):
	...
	web_form = nts.get_lazy_doc("Web Form", web_form)
...
def check_webform_perm(doctype, name):
	doc = nts.get_lazy_doc(doctype, name)
...
def get_link_options(web_form_name, doctype, allow_read_on_all_link_options=False):
	web_form: WebForm = nts.get_lazy_doc("Web Form", web_form_name)
